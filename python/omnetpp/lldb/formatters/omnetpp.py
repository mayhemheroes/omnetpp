...
def cOwnedObject_SummaryProvider(value, internal_dict):
    return f"{value.GetChildMemberWithName("name").GetSummary()[1:-1]} (owned by {value.GetChildMemberWithName("owner").GetSummary()})"

def cModule_SummaryProvider(value, internal_dict):
    return f"({value.GetChildMemberWithName("componentType").GetSummary()[1:-1]}) {value.GetChildMemberWithName("fullPath").GetSummary()[1:-1]}"
...
def cComponent_SummaryProvider(value, internal_dict):
    return f"{value.GetChildMemberWithName("name").GetSummary()[1:-1]}: {value.GetChildMemberWithName("componentType").GetSummary()[1:-1]}"

def cEvent_SummaryProvider(value, internal_dict):
    return f"{value.GetChildMemberWithName("name").GetSummary()[1:-1]} @{value.GetChildMemberWithName("arrivalTime").GetSummary()} pri:{value.GetChildMemberWithName("priority").GetValue()}"

def cMessage_SummaryProvider(value, internal_dict):
    return f"#{value.GetChildMemberWithName("messageId").GetValue()} {cEvent_SummaryProvider(value, internal_dict)} kind:{value.GetChildMemberWithName("messageKind").GetValue()}"

def cPacket_SummaryProvider(value, internal_dict):
    return f"{cMessage_SummaryProvider(value, internal_dict)} len:{value.GetChildMemberWithName("bitLength").GetValue()}bit dur:{value.GetChildMemberWithName("duration").GetSummary()}"
