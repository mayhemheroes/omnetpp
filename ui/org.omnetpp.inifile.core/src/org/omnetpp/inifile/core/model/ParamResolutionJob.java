    ...
    private IStatus tryRun(IProgressMonitor monitor) {
        ...
        try {
            ...
            if (monitor.isCanceled()) {
                ...
                return new Status(IStatus.CANCEL, InifileCorePlugin.PLUGIN_ID, USER_CANCELED, "", null);
            }
            else {
                Debug.println("ParamResolutionJob: stopped because input changed after "+(System.currentTimeMillis()-startTime)+"ms");
                return new Status(IStatus.CANCEL, InifileCorePlugin.PLUGIN_ID, DOC_CHANGED, "", null);
            }
        } finally {
            monitor.done();
        }
