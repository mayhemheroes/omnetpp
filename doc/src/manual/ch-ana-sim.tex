...
%   \ttt{omnetpp.sca}, and scalar files were always appended to rather than being
%   overwritten as in the 4.x version. When needed, the old behavior for scalar
%   files can be turned back on by setting \ttt{output-scalar-file-append=true} in
%   the configuration.
% \end{note}


\subsection{Global Result File Control}
\label{sec:ana-sim:global-result-file-control}

The creation of result files can be controlled with global switches that determine
whether vector or scalar files are written at all:

\begin{itemize}
  \item \fconfig{record-vector-results}: main switch for writing a vector file
  \item \fconfig{record-scalar-results}: main switch for writing a scalar file
\end{itemize}

To completely disable vector recording, use \ttt{record-vector-results = false}.
It is preferable to \ttt{**.vector-recording = false}, as the latter may still
create vector files containing only metadata. Similarly,
\ttt{record-scalar-results = false} should be preferred over
\ttt{**.scalar-recording = false}.
...
Individual result items can be controlled with per-object options that can be
configured individually for each \fprop{@statistic} and result item:

\begin{itemize}
  \item \fconfig{statistic-recording}: per-object option that enables or
    disables the recording of all items from a \fprop{@statistic}.
  \item \fconfig{vector-recording}: per-object option that controls the
    recording of output vectors.
  \item \fconfig{scalar-recording}: per-object option that controls the
    recording of non-vector items: scalars, statistic summaries and histograms.
  \item \fconfig{bin-recording}: per-object option that controls the recording
    of the bins of histograms.
\end{itemize}

The per-object options have similar syntaxes:
