...
\begin{itemize}
  ...
  \item \ttt{name}, \ttt{title}, \ttt{module}, etc. \textit{(optional)}: Provide input for the legend.
  \item \ttt{unit} \textit{(string, optional)}: The unit for the X-axis values. If present, it is required
...
\begin{itemize}
  ...
  \item \ttt{cycle\_seed}: Alters the sequence in which colors and markers are assigned to series.
  \item \ttt{xaxis\_unit}: If present, specifies the unit for the X-axis. If empty string, automatically selects the best unit.
  \item \ttt{yaxis\_unit}: If present, specifies the unit for the Y-axis. If empty string, automatically selects the best unit.
...
\begin{itemize}
  ...
  \item \ttt{image\_export\_dpi}: DPI setting, default 300. For raster image formats, the
...
\subsubsection{histogram\_bin\_edges\_from\_props()}
\label{cha:chart-api:omnetpp.scave.utils:histogram-bin-edges-from-props}

\begin{flushleft}
\ttt{histogram\_bin\_edges\_from\_props(values, unit, props)}
\end{flushleft}

\par Calculates histogram bin edges based on chart dialog properties.
This is a convenience wrapper around \ttt{histogram\_bin\_edges()}. See that function
for detailed documentation of the binning algorithm and parameters.

\par Parameters:

\begin{itemize}
  \item \ttt{values}: Input data array
  \item \ttt{unit}: Measurement unit of the input data
  \item \ttt{props} \textit{(dict)}: Properties from the chart configuration dialog, containing
    settings like "use\_manual", "manual\_edges", "num\_bins", "bin\_width",
    "range\_min", "range\_max", "round\_bin\_widths", "method".
...
\begin{itemize}
  \item bin\_edges: Array of bin edges
...
\begin{flushleft}
\ttt{histogram\_bin\_edges(values, num\_bins=None, bin\_width=None, range\_min=None, range\_max=None, method=None, round\_bin\_widths=True)}
\end{flushleft}

\par Enhanced histogram bin edge calculator with flexible parameter combinations
...
\begin{itemize}
  \item values: Input data array
  \item num\_bins: Number of bins to use. If round\_bin\_widths is True, this will be treated as a hint.
  \item bin\_width: Fixed width for each bin
  \item range\_min: Lower endpoint of histogram range
  \item range\_max: Upper endpoint of histogram range
  \item round\_bin\_widths: Choose "nice" bin widths such as 1, 2, 5 * 10{\textasciicircum}k.
  \item method: Binning method (used if num\_bins is None, default: 'auto' if None)

\end{itemize}


\par When parameters are provided (not None), they are respected.
...
\begin{itemize}
  \item ValueError: If parameters are overconstrained or incompatible
...
\begin{itemize}
  \item \ttt{df} \textit{(DataFrame)}: The DataFrame to operate on
  \item \ttt{columns} \textit{(string or list of strings)}: The column name or list of
...
\begin{flushleft}
\ttt{\small{Analysis.}export\_image(self, chart, wd, workspace, format="svg", target\_folder=None, filename=None, width=None, height=None, dpi=300, enforce=True, extra\_props=\{\}, suppress\_print=False)}
\end{flushleft}
