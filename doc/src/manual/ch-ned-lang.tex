...
existing gates is possible using the \fprop{reconnect} property, see
...
parameters of the current submodule or channel, and parameters of already
defined submodules, with the syntax \ttt{submodule.parametername} (or
\ttt{submodule[index].parametername}).

Unqualified parameter names refer to the nearest enclosing scope: inside a
submodule or channel block they refer to the submodule's or channel's own
parameters; outside, they refer to parameters of the compound
module.\footnote{In {\opp} 5.x, unqualified names inside submodule blocks
referred to the enclosing compound module. The current scoping rules are in
effect since {\opp} 6.0.} Use the \fkeyword{parent} qualifier to refer to
the compound module's parameters from within a submodule block, and the
\fkeyword{this} qualifier to make a reference to the local submodule explicit.
...
network Network
{
    parameters:
        double foo;
        double bar = foo;  // refers to Network.foo
    submodules:
        node[10]: Node {
            baz = parent.foo;  // refers to Network.foo
            datarate = amount / duration;  // refers to node[i]'s params
        }
    ...
}
\end{ned}

For the full scoping rules (including gates, submodule references, and loop
variables), see \ref{sec:ned-ref:referencing-parameters-and-loop-variables}.
...
module Node
{
    ...
    submodules:
        routing: Routing {
            ...
            gates:
                in[sizeof(parent.port)];  // set gate vector size
...
connected. This behavior can be overridden with the \fprop{@reconnect}
...
When a connection with the \fprop{@reconnect} property is encountered by the
...
The usefulness of \fprop{@reconnect} lies with inheritance, as it allows one
...
%% TODO use more intuitive module names? E.g. src, sink, tweaker
...
\begin{itemize}
  ...
  \item \fprop{@omittedTypename}: For submodules and connections with parametric
    type (\fkeyword{like}), specifies the type name to use (or suppresses
    creation) when the type name expression evaluates to an empty string. See
    \ref{sec:ned-ref:omittedtypename-property}.
  \item \fprop{@reconnect}: On a connection, allows reconnecting gates that
    are already connected, by first disconnecting them. See
    \ref{sec:ned-lang:reconnecting-gates}, \ref{sec:ned-ref:reconnect-property}.
...
\begin{itemize}
  ...
  \item \fprop{@enum(<strings>)}: Defines a list of valid values for the
    parameter. See \ref{sec:ned-ref:enum-property}.
  \item \fprop{@class(<classname>)}: For \fkeyword{object} parameters, specifies
    the expected C++ class of the parameter's value. See
    \ref{sec:ned-ref:class-param-property}.
\end{itemize}
