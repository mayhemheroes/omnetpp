...
The \fopt{-l} option instructs \fprog{opp\_run} to load the specified shared library.
The \fopt{-l} option will be explained in detail in section
\ref{sec:run-sim:loading-extra-libraries}.

\begin{note}
Normal simulation executables, like the aforementioned \ttt{fifo}, are also able to
load additional shared libraries in the same manner. Moreover, \fprog{opp\_run} is
...
The default ini file is \ffilename{omnetpp.ini} and is loaded if no other ini
file is specified on the command line.

Ini files can be specified either as plain arguments or with the \fopt{-f}
...
\begin{itemize}
  \item by using the \ttt{NEDPATH} environment variable
  \item by using the \fopt{-n} command-line option
...
\begin{enumerate}
  \item {\opp} checks for the NED path specified on the command line with the \fopt{-n} option
...
dynamically, or loaded at runtime.\footnote{Via the \fopt{-l}
...
simulation program can be listed by running it with the \fopt{-h userinterfaces}
option.
...
\begin{itemize}
  \item \fconfig{sim-time-limit} : Limits the duration for which the simulation should run (in simulation time).
  \item \fconfig{cpu-time-limit} : Limits the maximum CPU time that the simulation can use.
  \item \fconfig{real-time-limit} : Limits the actual duration of the simulation (in real time).
\end{itemize}

\begin{note}
Although \fconfig{cpu-time-limit} and \fconfig{real-time-limit} may appear
similar, in practice, \fconfig{cpu-time-limit} is almost always needed. Its
alternative, \fconfig{real-time-limit}, simply measures elapsed time (wall-clock
...
number of computationally intensive jobs, \fconfig{real-time-limit} may
...
This can be achieved by calling the \ffunc{endSimulation()} method.
...
\begin{itemize}
  \item \fconfig{record-eventlog} : Turns on the recording of simulator
        events into an event log file. The resulting \ttt{.elog} file can be
        analyzed later in the IDE with the Sequence Chart tool.

  \item \fconfig{record-scalar-results} :  This option can be used to completely
        disable writing output scalar files (\ttt{.sca}). This is preferable to
        \ttt{**.scalar-recording=false} when the goal is to disable writing \ttt{.sca} files
        entirely, as the latter may still create \ttt{.sca} files containing only metadata.

  \item \fconfig{record-vector-results} : This option can be used to completely
        disable writing output vector files (\ttt{.vec}). This is preferable to
        \ttt{**.vector-recording=false} when the goal is to disable writing \ttt{.vec} files
        entirely, as the latter may still create \ttt{.vec} files containing only metadata.
...
\begin{commandline}
$ ./fifo --record-eventlog=true --record-vector-results=false
\end{commandline}
...
Luckily, this problem is easy to identify, as all user interfaces display the
...
If the simulation is leaking objects derived from \cclass{cOwnedObject}, these
...
\begin{itemize}
  ...
  \item \textbf{AddressSanitizer}, \textbf{LeakSanitizer}, and several other
    ...
    compile mode (\ttt{MODE=sanitize}) that builds the {\opp} libraries and
...
for them to calculate yet. At the end of the simulation, the \ffunc{finish()}
...
\fopt{-r}, which are used to select which simulations to perform. (These options
...
\begin{itemize}
  \item For Express mode: \fconfig{cmdenv-performance-display}, \fconfig{cmdenv-status-frequency}
  \item For Normal mode: \fconfig{cmdenv-event-banners}, \fconfig{cmdenv-event-banner-details},
...
\begin{itemize}
   \item \ttt{ev/sec} indicates \textit{performance}, i.e., how many events are
   ...
   \item \ttt{simsec/sec} shows the \textit{relative speed} of the simulation, i.e.,
   ...
   \item \ttt{ev/simsec} is the \textit{event density}, i.e., how many events are
...
\begin{itemize}
   \item{\ttt{Created}: the total number of message objects created since the
   ...
   \item{\ttt{Present}: the number of message objects currently present
     ...
     the messages in the Future Event Set (FES)\index{FES}.}
...
Qtenv\index{Qtenv} is a graphical user interface for running simulations. Qtenv
...
Simulations running under Qtenv accept all general command-line and
configuration options, including \fopt{-c} and \fopt{-r}. The configuration
options specific to Qtenv include:

\begin{itemize}
  \item \fconfig{qtenv-default-config}: Specifies which config Qtenv should
    automatically set up on startup. The default is to ask the user. This option
    is equivalent to the \fopt{-c} command-line option.

  \item \fconfig{qtenv-default-run}: Specifies which run (of the default config,
    see qtenv-default-config) Qtenv should automatically set up on startup. The
    default is to ask the user. This option is equivalent to the \fopt{-r}
    command-line option.

  \item \fconfig{qtenv-extra-stack}: Specifies the additional amount of stack
...
Save the above into a text file called \ttt{runAloha}. Then give it
