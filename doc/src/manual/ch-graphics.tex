...
it was embedded in the \ffunc{handleMessage()} function, enclosed in \ttt{if
(ev.isGUI())} blocks. This approach was less than ideal because the
...
placed inside \ffunc{handleMessage()} could potentially waste a significant
...
dedicated method called \ffunc{refreshDisplay()}. Using this method is more
efficient than embedding visualization code into \ffunc{handleMessage()},
because \ffunc{refreshTheDisplay()} is called only as often as necessary
by the graphical user interface to keep the display up to date.

\ffunc{refreshDisplay()} is declared on \cclass{cModule} as:
...
When and how is \ffunc{refreshDisplay()} invoked? Generally, it is invoked right
...
One useful accessory to \ffunc{refreshDisplay()} is the \ffunc{isExpressMode()}
method of \cclass{cEnvir}. It returns true if the simulation is running under a
...
module, with \ffunc{handleMessage()}-based visualization, the model may have
inconsistent visualization until the second module also processes an event and
updates its displayed state. With \ffunc{refreshDisplay()}, this does not happen
...
\ffunc{refreshDisplay()} allows one to completely separate the two.

\subsection{Why is \ffunc{refreshDisplay()} const?}
\label{sec:graphics:refreshdisplay-constness}

Code in \ffunc{refreshDisplay()} should never alter the state of the simulation
because that would destroy repeatability due to the fact that the timing and
frequency of \ffunc{refreshDisplay()} calls are completely unpredictable from
the simulation model's point of view. The fact that the method is declared
\ttt{const} gently encourages this behavior.

If visualization code makes use of internal caches or maintains some other
mutable state, such data members can be declared \ttt{mutable} to allow
\ffunc{refreshDisplay()} to change them.
...
An animation speed request may be submitted using the \ffunc{setAnimationSpeed()}
...
Display strings\index{display strings} are concise textual descriptions that
...
Display strings are usually specified in NED's \fprop{@display} property, but it
...
Display strings are specified in \fprop{@display} properties in NED files. The
...
Display strings can be placed in the \fkeyword{parameters} section of module and
...
Commas and semicolons inside \ttt{\$\{...\}} do not need to be escaped with a backslash.

\subsection{Inheritance}
\label{sec:graphics:displaystring-inheritance}

At runtime, every module and channel object has a single display string object
that controls its appearance in various contexts. The initial value of this
display string object comes from merging the \fprop{@display} properties
occurring at various places in NED files. This section describes the rules for
merging \fprop{@display} properties to create the module or channel's display
string.
...
\begin{enumerate}
  \item \tbf{Inheriting.} If a tag or tag argument is present in the base
    ...
    Example: \ttt{"i=block/sink"} (base) + \ttt{"p=20,40;i=,red"} (current)
    $\rightarrow$ \ttt{"p=20,40;i=block/sink,red"}
  \item \tbf{Overwriting.} If a tag argument is present in both the base and
    current display strings, the tag argument in the current display string will
    take priority. Example: \ttt{"b=40,20,oval"} + \ttt{"b=,30"} $\rightarrow$
    \ttt{"b=40,30,oval"}
  \item \tbf{Erasing.} If the current display string contains a tag argument
    ...
    Example: \ttt{"i=block/sink,red"} + \ttt{"i=,-"} $\rightarrow$
    \ttt{"i=block/sink"}
\end{enumerate}
...
\begin{itemize}
  ...
  \item \ttt{it} -- icon transform (rotation, flip, scale)
...
available, look into the \ttt{images/} folder in the {\opp} installation. The
...
One can specify the icon with the \ttt{i} tag. The icon name should be given
with the name of the subfolder under \ttt{images/}, but without the file name
extension. The size may be specified with the icon name suffix (\ttt{\_s} for
very small, \ttt{\_vl} for very large, etc.), or in a separate \ttt{is} tag.
...
example, and can be set with the \ttt{i2} tag. Icons suitable for use with
\ttt{i2} are in the \ttt{status/} category.
...
\ttt{b} tag to create geometric shapes. Currently, \ttt{oval} and
\ttt{rectangle} are supported.

The following example displays an oval shape with a size of 70x30, a 4-pixel
...
The \ttt{p} tag allows one to define the position of a submodule or otherwise affect its placement.

\begin{note}
If the \ttt{p} tag is missing or doesn't specify the position, {\opp} will use a
...
row, a column, a matrix, a ring or a centered ring. The rest of the arguments in the \ttt{p}
...
\begin{itemize}
  \item Row: \ttt{p=$x$,$y$,r,$deltaX$[,$numCols$,$deltaY$]} (A row of modules with $deltaX$ units between
            the modules. If $numCols$ is given, wraps into multiple rows after that many columns.)
  \item Column: \ttt{p=$x$,$y$,c,$deltaY$[,$numRows$,$deltaX$]} (A column of modules with $deltaY$ units
            between the modules. If $numRows$ is given, wraps into multiple columns after that many rows.)
  ...
  \item Ring: \ttt{p=$x$,$y$,ri,$rx$,$ry$} (A ring (oval) with $rx$ and $ry$ as the horizontal and vertical radius.
            The $(x,y)$ coordinates correspond to the upper left corner of the bounding rectangle.)
  \item Centered ring: \ttt{p=$x$,$y$,cr,$rx$,$ry$} (Same as ring, but the $(x,y)$ coordinates
            correspond to the center of the ring.)
...
range. This can be done with the \ttt{r} tag.
...
If a module contains a queue object (\cclass{cQueue}), it is possible to display
...
the \ffunc{setName()} method) in the \ttt{q} display string tag. {\opp} finds
...
using the \ttt{t} tag in the display string. The tag allows one to specify the
placement (left, right, above) and the color of the text. To display text in a
tooltip, use the \ttt{tt} tag.
...
\begin{itemize}
  ...
  \item \ttt{t} -- text
  \item \ttt{tt} -- tooltip
  \item \ttt{m} -- routing constraint (direction of departure and arrival)
\end{itemize}
...
\subsubsection{Connection Routing}
\label{sec:graphics:connection-routing}

The \ttt{m} tag controls how a connection is routed between two modules.
The routing rules differ depending on whether the connection goes from
a submodule to its parent module, or between two submodules.

\textbf{Submodule to parent.}
The syntax is \ttt{m=<dir>}, where \textit{dir} is a cardinal direction
letter (\ttt{n}, \ttt{e}, \ttt{s}, \ttt{w}) that specifies which edge of
the parent module the connection should reach. If omitted or set to \ttt{a}
(auto), the nearest edge is used.

\begin{ned}
child.out++ --> { @display("m=n"); } --> out++;   // go to north edge of parent
child.out++ --> { @display("m=w"); } --> out++;   // go to west edge of parent
child.out++ --> out++;                            // nearest edge (auto)
\end{ned}

\textbf{Submodule to submodule.}
The syntax is \ttt{m=<srcDir>,<destDir>}, where each direction specifies
the preferred axis along which the connection should leave the source or enter
the destination module. Accepted values are \ttt{h} (horizontal, i.e.\ leave
or enter via the east or west edge) and \ttt{v} (vertical, i.e.\ via the
north or south edge). The values \ttt{e} and \ttt{w} are synonyms for
\ttt{h}; \ttt{n} and \ttt{s} are synonyms for \ttt{v}. Either or both
directions may be left empty (unconstrained).

\begin{ned}
a.out --> { @display("m=h,v"); } --> b.in;   // leave horizontally, arrive vertically (L-shape)
a.out --> { @display("m=v,h"); } --> b.in;   // leave vertically, arrive horizontally (L-shape)
a.out --> { @display("m=h"); } --> b.in;     // leave horizontally, arrive auto
a.out --> { @display("m=,v"); } --> b.in;    // leave auto, arrive vertically
\end{ned}

\textbf{Routing rules.}
The following rules are applied in priority order:
\begin{enumerate}
  \item \textbf{Overlap rule (strongest).} If the bounding rectangles of the
    source and destination modules have meaningful overlap in either the x or
    y direction, a straight horizontal or vertical line is drawn through the
    overlap zone, regardless of any specified directions.
  \item \textbf{L-shape rule.} If there is no overlap and at least one
    direction is specified (and not the invalid same-axis combination
    \ttt{h,h} or \ttt{v,v}), an L-shaped route with one bend point is
    produced. The constrained endpoint exits or enters along the specified
    axis; the unconstrained endpoint uses the other axis as implied by the
    geometry.
  \item \textbf{Fallback.} If no direction is specified, or the combination
    is invalid (\ttt{h,h} or \ttt{v,v}), a straight line is drawn between
    the two modules.
\end{enumerate}

\textbf{Bundling.}
When there are multiple connections between the same pair of modules
(a \textit{bundle}), the connections are spread apart to avoid overlapping.
For straight segments, they are distributed within the overlap zone.
For L-shaped routes, the same offset is applied to both the exit and entry
edges, resulting in parallel L-shapes. Note that bundling offset is optional
in Qtenv (only turned on when requested in the GUI).

\textbf{Other modes.}
The value \ttt{m=a} selects automatic routing (equivalent to leaving both
directions empty). The value
\ttt{m=m,\textit{sx},\textit{sy},\textit{dx},\textit{dy}} selects manual
mode, where the connection endpoints are placed at the specified percentages
of the source and destination module's bounding rectangles.
...
\ttt{\$\{...\}} and \ttt{\$foo} syntaxes, respectively. To add a literal
...
A color may be given in several forms. One form is English names: \ttt{blue},
\ttt{lightgrey}, \ttt{wheat}, etc. The list includes all standard SVG color
names.
...
It will map the message kind to a color. (See the \ffunc{getKind()} method of
\cclass{cMessage}.)

\subsubsection{Icon Colorization}
\label{sec:graphics:displaystring-icon-colorization}

The \ttt{"i="} display string tag allows for colorization of icons. It accepts a
...
brightness (e.g., \ttt{\#808080} mid-grey, \ttt{\#008000} mid-green).
...
to \ttt{\textasciitilde/.bashrc} or \ttt{\textasciitilde/.bash\_profile} will
...
Sizes are encoded into the icon name's suffix: \ttt{\_vl}, \ttt{\_l}, \ttt{\_s},
\ttt{\_vs}. In display strings, one can either use the suffix
(\ttt{"i=device/router\_l"}) or the \ttt{"is"} (icon size) display string tag
(\ttt{"i=device/router;is=l"}), but not both at the same time (we recommend
using the \ttt{is} tag).


\subsubsection{Icon Transform}
\label{sec:graphics:icon-transform}

The \ttt{"it"} (icon transform) display string tag allows rotating, flipping,
and scaling the module icon. The syntax is
\ttt{it=<degrees>,<flip>,<scaleX>,<scaleY>}, where all arguments are optional:

\begin{itemize}
  \item \ttt{degrees} -- rotation in degrees, clockwise (default: 0)
  \item \ttt{flip} -- \ttt{h} for horizontal flip, \ttt{v} for vertical flip,
    \ttt{b} for both (default: none)
  \item \ttt{scaleX} -- horizontal scaling factor (floating-point, default: 1.0)
  \item \ttt{scaleY} -- vertical scaling factor (floating-point, default: same as scaleX)
\end{itemize}

Transformations are applied in the following order: scale and flip first, then
rotation. The transform only affects the main icon (\ttt{"i"} tag), not the
shape (\ttt{"b"} tag) or the overlay icon (\ttt{"i2"} tag).

Examples:

\begin{ned}
@display("i=misc/antenna;it=45");       // rotate 45 degrees
@display("i=misc/antenna;it=,h");       // flip horizontally
@display("i=misc/antenna;it=90,v,2");   // scale 2x, flip vertically, rotate 90
@display("i=misc/antenna;it=,,1.5,0.5"); // stretch: 1.5x wide, 0.5x tall
\end{ned}
...
the \ttt{"p="} tag will be automatically placed by the algorithm.
...
Display strings are stored in \cclass{cDisplayString} objects inside channels,
modules, and gates. \cclass{cDisplayString} also allows one to manipulate the
string.

As far as \cclass{cDisplayString} is concerned, a display string (e.g.,
\ttt{"p=100,125;i=cloud"}) is a string that consists of several \textit{tags}
...
the component's \ffunc{getDisplayString()} method.
...
The following example updates a display string to contain the \ttt{p=40,20} and
\ttt{i=device/cellphone} tags:
...
down" or "Connection established") by calling the \ffunc{bubble()} method of
...
corresponding C++ types for figures are \cclass{cCanvas} and \cclass{cFigure}.
\cclass{cFigure} is an abstract base class, and different types of figures are
represented by various subclasses of \cclass{cFigure}.

Figures can be defined statically in NED files using \fprop{@figure} properties,
...
A canvas is represented by the \cclass{cCanvas} C++ class. The default canvas of
a module can be accessed with the \ffunc{getCanvas()} method of
\cclass{cModule}. For example, a toplevel submodule can obtain the network's
...
New canvases can be created by instantiating new \cclass{cCanvas} objects. For example:
...
The base class of all figure classes is \cclass{cFigure}. The class hierarchy is
...
contain child figures, not just dedicated ones like \cclass{cGroupFigure}.

Every figure has a name string, inherited from \cclass{cNamedObject}. Since
...
Child figures can be added to a figure using the \ffunc{addFigure()} method, or
inserted into the child list relative to a sibling using the
\ffunc{insertBefore()} and \ffunc{insertAfter()} methods. \ffunc{addFigure()}
has two variants: one for appending and one for inserting at a specific
position. Child figures can be accessed by name using \ffunc{getFigure(name)},
or enumerated by index in the child list using \ffunc{getFigure(k)} and
\ffunc{getNumFigures()}. The index of a child figure can be obtained using
\ffunc{findFigure()}. The \ffunc{removeFromParent()} method can be used to
remove a figure from its parent.

For convenience, \cclass{cCanvas} also provides methods like
\ffunc{addFigure()}, \ffunc{getFigure()}, and \ffunc{getNumFigures()} for
...
(\ffunc{getFigureByPath()}) and to find a figure by its non-hierarchical name
anywhere in a figure subtree (\ffunc{findFigureRecursively()}).

The \ffunc{dup()} method of figure classes only duplicates the figure it is
called on. (The duplicate will not have any children.) To clone a figure
including its children, use the \ffunc{dupTree()} method.
...
Figures are defined in NED by adding \fprop{@figure} properties to a module
definition. The hierarchical name of the figure goes into the property index,
enclosed in square brackets after \ttt{@figure}. The parent of the figure must
already exist. For example, when defining \ttt{foo.bar.baz}, both \ttt{foo} and
\ttt{foo.bar} must have already been defined in the NED file.

The type and various attributes of the figure are specified in the property body
as key-value pairs. For example, \ttt{type=line} creates a \cclass{cLineFigure},
\ttt{type=rectangle} creates a \cclass{cRectangleFigure}, \ttt{type=text}
creates a \cclass{cTextFigure}, and so on. The list of accepted types is
provided in appendix \ref{cha:figure-definitions}. Additional attributes
correspond to getters and setters of the C++ class denoted by the \ttt{type}
attribute.
...
the child order and the \cclass{cFigure} attribute called Z-index, with the
...
\ffunc{setZIndex()} to set the Z-index of a figure, \ffunc{getZIndex()} to get
the Z-index of a figure, \ffunc{getEffectiveZIndex()} to get the effective
Z-index of a figure, \ffunc{insertAbove()} and \ffunc{insertBelow()} to insert a
figure above or below another figure, \ffunc{isAbove()} and \ffunc{isBelow()} to
check if a figure is above or below another figure, and \ffunc{raiseAbove()},
\ffunc{lowerBelow()}, \ffunc{raiseToTop()}, and \ffunc{lowerToBottom()} to raise
...
Given a point with coordinates $(x, y)$, the transformed version of the point
...
The \cclass{cFigure::Transform} class represents transformation matrices in {\opp}.

A \cclass{cFigure::Transform} matrix can be initialized using various methods.
One way is to assign values directly to the \ttt{a}, \ttt{b}, \ttt{c}, \ttt{d},
\ttt{t1}, and \ttt{t2} members. Another way is to use the six-argument
constructor. It is also possible to start from the identity transform (created
by the default constructor) and apply transformations using the \ffunc{scale()},
\ffunc{rotate()}, \ffunc{skewx()}, \ffunc{skewy()}, and \ffunc{translate()}
...
To transform a point, represented by the \cclass{cFigure::Point} class, one can
use the \ffunc{applyTo()} method of \cclass{cFigure::Transform}. The following
...
accessed using the \ffunc{isVisible()} and \ffunc{setVisible()} member functions
of \cclass{cFigure}.
...
via \ttt{setVisible(false)} cannot be displayed using tags. Also, hiding a figure
using the tag filter hides its figure subtree as well.

Tags can be assigned to figures using the \ffunc{setTags()}
method, which takes a single string of space-separated tags (tags may not
contain spaces). The methods \ffunc{getTags()} and \ffunc{setTags()} can
...
Figures can be assigned a tooltip text using the \ffunc{setTooltip()} method.
...
object, the object can be set using the \ffunc{setAssociatedObject()} method.
...
The \cclass{cPolygonFigure} displays a closed polygon, which is determined by a sequence of points.
...
using the \ffunc{setSmooth()} method.

The points can be set using the \ffunc{setPoints()} method, which takes a \ttt{std::vector<Point>},
or added one-by-one using the \ffunc{addPoint()} method. Elements in the point list can be
read and overwritten using the \ffunc{getPoint()} and \ffunc{setPoint()} methods. One can also
insert and remove points using the \ffunc{insertPoint()} and \ffunc{removePoint()} methods.
...
can be set using the \ffunc{setFillRule()} method. The line join style can be set using
the \ffunc{setJoinStyle()} method.

Line and fill color, and several other properties are inherited from
the \cclass{cAbstractShapeFigure}.

Here is an example of a smoothed polygon that also demonstrates
the use of the \ffunc{setPoints()} method:
...
The \cclass{cPathFigure} displays a ``path'', which is a complex shape or line modeled after SVG
...
The \cclass{cPathFigure} can accept the path in string form (\ffunc{setPath()}),
...
and \ffunc{getPathItem(k)} methods. The latter returns a pointer to a
...
\ffunc{setCapStyle()}, and \ffunc{setFillRule()} methods.

The \cclass{cPathFigure} has one more property, an $(dx,dy)$ offset, which
exists to simplify the implementation of the \ffunc{move()} method. The offset
...
\ffunc{move()} updates the offset. The offset can be set with
\ffunc{setOffset()}.
...
The \cclass{cAbstractTextFigure} is an abstract base class for figures that
...
if the anchor is \ttt{ANCHOR\_CENTER}, then the text is centered on the point;
if the anchor is \ttt{ANCHOR\_N}, then the text will be drawn so that its top
...
\ffunc{setPosition()} and \ffunc{setAnchor()}. The anchor defaults to
\ttt{ANCHOR\_CENTER}.

The font can be set with the \ffunc{setFont()} member function, which takes
\cclass{cFigure::Font}, a class that encapsulates typeface, font style, and
size. The color can be set with \ffunc{setColor()}. The displayed text can
also be partially transparent. This is controlled with the \ffunc{setOpacity()}
member function, which accepts a \ttt{double} in the range of $[0,1]$, where $0$ means
...
The \cclass{cTextFigure} displays text which is affected by zooming and
...
The \cclass{cLabelFigure} displays text that is unaffected by zooming or
transformations, except its position. The font, color, position, anchoring, and
other properties are inherited from \cclass{cAbstractTextFigure}. The angle of
the label can be set with the \ffunc{setAngle()} method. A zero angle means
...
in the NED file via \fprop{@figure}.
\end{note}
...
The \cclass{cAbstractImageFigure} is an abstract base class for image figures.
...
if the anchor is \ttt{ANCHOR\_CENTER}, the image is centered on the point;
if the anchor is \ttt{ANCHOR\_N}, the image will be drawn so that its top
...
\ffunc{setAnchor()}. The anchor defaults to \ttt{ANCHOR\_CENTER}.

By default, the figure's width and height will be taken from the image's
dimensions in pixels. This can be overridden with the \ffunc{setWidth()} and
\ffunc{setHeight()} methods, causing the image to be scaled. Using
\ttt{setWidth(0)} or \ttt{setHeight(0)} will reset the default (automatic) width
and height.
...
can be set with the \ffunc{setInterpolation()} method, which defaults to
...
The \cclass{cImageFigure} displays an image, typically an icon or a background
...
the \cclass{cImageFigure} fully obeys transforms and zoom.
...
The \cclass{cIconFigure} displays a non-zooming image, loaded from the {\opp}
image path. Positioning and other properties are inherited from
\cclass{cAbstractImageFigure}.

The \cclass{cIconFigure} is not affected by transforms or zoom, except its
position. It can still be resized, though, via the \ffunc{setWidth()} and
\ffunc{setHeight()} methods.

The following example displays an icon similar to the way the
\ttt{"i=block/sink,gold,30"} display string tag would, and makes it slightly
transparent:
...
The \cclass{cPixmapFigure} displays a user-defined raster image. A pixmap
...
The \cclass{cFigure::Pixmap} stores a rectangular array of 32-bit RGBA pixels,
...
Pixels can be directly accessed using \ffunc{pixel(x,y)}.
...
The \cclass{cGroupFigure} is for the sole purpose of grouping its children. It
...
The \cclass{cPanelFigure} is similar to \cclass{cGroupFigure} in that it is also
...
The \cclass{cPanelFigure} sets up an axis-aligned, unscaled coordinate system for
...
Unlike \cclass{cGroupFigure}, which has its own position attribute,
...
The following example demonstrates the \cclass{cPanelFigure} behavior. It
...
\begin{center}
\includepng{figures/figure-panel}
\end{center}
...
\textit{Compound figures} are \cclass{cFigure} subclasses that
...
be instantiated from \fprop{@figure} properties.
...
To enable the new C++ class to be used with \fprop{@figure},
...
If the figure should be able to take values from \fprop{@figure} properties,
the class needs to override the \ffunc{parse(cProperty*)} method,
and possibly also \ffunc{getAllowedPropertyKeys()}. We recommend
...
\ffunc{refreshDisplay()} is declared in \cclass{cFigure} as:
...
\cclass{QGraphicsItem}s on a \cclass{QGraphicsView}. To render a
...
\ttt{omnetpp::qtenv::LineFigureRenderer}. If such a class exists and is a
...
On the other hand, osgEarth (osgearth.org) is a geospatial SDK and terrain
...
\begin{note}
\tbf{What is a scene graph?} A scene graph is a tree-like directed graph data
...
\cclass{cOsgCanvas} wraps a scene graph, along with hints for the GUI on how to
...
It is important to note that the simulation model can only manipulate the scene
...
Every module has a built-in (default) \cclass{cOsgCanvas}, which can be accessed
using the \ffunc{getOsgCanvas()} method of \cclass{cModule}. For example, a
...
Additional \cclass{cOsgCanvas} instances can be created using the \ttt{new} keyword:
...
Once a scene graph has been assembled, it can be set on the \cclass{cOsgCanvas}
using the \ffunc{setScene()} method.
...
visualization. There is no need to call \ffunc{setScene()} again or let {\opp}
know about the changes in any other way.

\subsubsection{Viewer Hints}
\label{sec:graphics:osgcanvas-viewer-hints}

There are several hints that the 3D viewer can take into account when displaying
...
\begin{itemize}
  ...
  \item \tbf{Viewer style:} The viewer style can be set using the
    \ffunc{setViewerStyle()} method. It determines the default hints for a
    scene. The choices are \ttt{STYLE\_GENERIC}, which should be set for generic
    (non-osgEarth) scenes (default), and \ttt{STYLE\_EARTH} for osgEarth scenes.
    As a rule of thumb, \ttt{STYLE\_EARTH} should only be used when the model is
    loading \ttt{.earth} files.

  \item \tbf{Camera manipulators:} The OSG viewer makes use of camera
    manipulators, which map mouse and keyboard gestures to camera movement.
    Users can specify a manipulator using the \ffunc{setCameraManipulatorType()}
    method. Several camera manipulators are available: \ttt{CAM\_TERRAIN}, which
    is suitable for flying above an object or terrain; \ttt{CAM\_OVERVIEW}, which
    is similar to the terrain manipulator but does not allow rolling or looking
    up (users can only see the object from above); \ttt{CAM\_TRACKBALL}, which
    allows unrestricted movement centered around an object; and \ttt{CAM\_EARTH},
    ...
    (\ttt{CAM\_AUTO}) based on the viewer style (\ttt{CAM\_OVERVIEW} or
    \ttt{CAM\_EARTH}).

  \item \tbf{Scene rendering:} Users can set the default background color for
    non-osgEarth scenes using the \ffunc{setClearColor()} method. It is also
    possible to set the distances of the near and far clipping planes using the
    \ffunc{setZNear()} and \ffunc{setZFar()} methods. These distances determine
  ...
  \item \tbf{Viewpoint and field of view:} Users can set default viewpoints
    using the \ffunc{setGenericViewpoint()} method by specifying the camera
    position, focal point, and ``up'' direction as parameters. For osgEarth
    scenarios, users can use the \ffunc{setEarthViewpoint()} method to set the
    ...
    \ffunc{setFieldOfViewAngle()} method.
\end{itemize}
...
{\opp} provides a wrapper node, \cclass{cObjectOsgNode}, that associates its
children with a particular {\opp} object (a descendant of \cclass{cObject}),
making them selectable in the 3D viewer. To use \cclass{cObjectOsgNode}, create
...
\ffunc{resolveResourcePath()} method.

The \ffunc{resolveResourcePath()} method of modules and channels accepts a file
...
or access an icon with its full name (e.g., \ttt{block/sink}).

Here is an example that attempts to load a \ttt{car.osgb} model file:
...
{\opp} detects the OSG and osgEarth libraries and defines the \fmac{WITH\_OSG}
...
simulations with only a subset of these libraries, including \ttt{osg},
\ttt{osgGA}, \ttt{osgViewer}, \ttt{osgQt}, \ttt{osgEarth}, and
\ttt{osgEarthUtil}. If additional OSG and osgEarth libraries are needed, they
must be linked to the model as well. To link these libraries, add the following
code fragment to the \ttt{makefrag} file of the project:
...
The \ttt{ifneq()} statements ensure that \ttt{LIBS} is only updated if {\opp}
has detected the presence of OSG/osgEarth.

\subsection{Using OSG}
\label{sec:graphics:using-osg}

OpenScenegraph is a large library with 16+ namespaces and 40+ \cclass{osg::Node}
...
To display a 3D model in the canvas of a compound module, an \cclass{osg::Node}
must be provided as the root of the scene.

One method of obtaining such a \ttt{Node} is to load it from a file containing
the model. This can be done using the \ffunc{osgDB::readNodeFile()} method (or
...
\begin{note}
\tbf{Where to get model files:} While OpenSceneGraph recognizes and can load a
...
This line scales the original cow model in \ttt{cow.osgb} to 200\%, rotates it
...
Note that these modifiers operate directly on the model data and are independent
...
Shapes can also be built programmatically using the \cclass{osg::Geode},
\cclass{osg::ShapeDrawable}, and \cclass{osg::Shape} classes.

To create a shape, start by creating an \cclass{osg::Shape}. The
\cclass{osg::Shape} class is abstract and has several subclasses, such as
\cclass{osg::Box}, \cclass{osg::Sphere}, \cclass{osg::Cone},
\cclass{osg::Cylinder}, and \cclass{osg::Capsule}. This object represents the
abstract definition of the shape and cannot be rendered on its own. To render
the shape, create an \cclass{osg::ShapeDrawable} for it. However, an
\cclass{osg::ShapeDrawable} by itself cannot be added to the scene as it is not
an \cclass{osg::Node}. To add it to the scene, the \cclass{osg::ShapeDrawable}
must be added to an \cclass{osg::Geode} (a ``geometry node''). Finally, add the
\cclass{osg::Geode} to the scene.
...
Note that the same \cclass{osg::Shape} instance can be used to create multiple
\cclass{osg::Shape\-Drawable}s, and a single \cclass{osg::Shape\-Drawable} can be
added to multiple \cclass{osg::Geode}s to display it in multiple places or sizes
...
Models can be positioned and oriented by making them children of an
\cclass{osg::Position\-Attitude\-Transform}. This node provides methods to set the
position, orientation, and scale of its children. Orientation is done using
quaternions (\cclass{osg::Quat}). To construct a quaternion, specify an axis of
...
If the image has transparent parts, the following lines should be added:\footnote{These lines
...
The \cclass{osg::Geode} should be made a child of an
\cclass{osg::AutoTransform}, which applies the correct transformations for the
...
The \ttt{autoTransform} can now be made a child of the \ttt{modelToTransform}
...
To draw a line between two points in the scene, create a \cclass{osg::Vec3Array}
to store the points, an \cclass{osg::DrawArrays} to specify the part of the
array to be drawn, and an \cclass{osg::Geometry} to connect them.
...
To change the visual properties of the line, modify the \cclass{osg::StateSet}
of the \cclass{osg::Geometry}. For example, to change the line width:
...
It is recommended to set an appropriate \cclass{osg::Material} to control the
...
because any modification made to an \cclass{osg::Node} is by default propagated
to all of its children, including transformations, render state variables, and
other flags.

For simple scenes, having an \cclass{osg::Group} as the root node and making
...
a child of an \cclass{osg::Position\-Attitude\-Transform} node.
...
If the node loaded by \ffunc{readNodeFile()} contains animations (sometimes
called actions), the \ttt{osgAnimation} module is capable of playing them back.
...
\cclass{osgAnimation::AnimationManager}, with methods such as
\ffunc{playAnimation()}, \ffunc{stopAnimation()}, \ffunc{isPlaying()}, etc.
Animation managers can be found among the descendants of the loaded
\cclass{osg::Node}s that are animated using a custom \cclass{osg::NodeVisitor}:
...
callback of type \cclass{osgAnimation::AnimationManagerBase}. Its result is a
new \cclass{osgAnimation::BasicAnimationManager} created from the base.

This new \ttt{animationManager} must be set as an update callback on the
\ttt{objectNode} to be able to drive the animations. Then, any animation in the
list returned by \ffunc{getAnimationList()} can be set up as needed and played.
...
An easy way to access it is via the \ffunc{getOrCreateStateSet()} method of the
drawable node. An \cclass{osg::StateSet} encapsulates a subset of the OpenGL
...
culling, depth and stencil options, and many more \cclass{osg::StateAttributes}.

The following example enables blending for a node and sets up a transparent,
colored material to be used for rendering it through its \cclass{osg::StateSet}.
...
\ttt{TRANSPARENT\_BIN} by setting a rendering hint on their
\cclass{osg::StateSet}. This ensures that they will be drawn after all fully
...
using the \cclass{osg::Depth} attribute.
...
loading \ttt{.earth} files to \ttt{osgDB::readNodeFile()}. An \ttt{.earth} file
...
\ffunc{osgEarth::MapNode::findMapNode()} function. This node serves as the data
model that contains all the data specified in the \ttt{.earth} file.
...
(\ttt{projected}) or rendered as a geoid (\ttt{geocentric}).

The source of the terrain's texture is specified by \ttt{image} tags. Many
...
refer to the online references of osgEarth or use one of the sample \ttt{.earth} files
included with it.

The following partial \ttt{.earth} file places a label over Los Angeles, an extruded
...
command line tool called \fprog{osgearth\_package}. The resulting package,
unlike map cache, will also be redistributable.

With the appropriate arguments, \fprog{osgearth\_package} can download the tiles
that make up the map and arrange them in a standardized, self-contained package.
It also creates a corresponding \ttt{.earth} file that can be used later, just like
any other.
...
The \ttt{-{}-tms boston.earth} arguments indicate that we want to create a
package in TMS format from the input file boston.earth. The \ttt{-{}-out
offline-tiles} argument specifies the output directory.

The \ttt{-{}-bounds} argument specifies the rectangular area of the map to
...
The \ttt{-{}-max-level 18} argument sets the maximum level of detail to be
...
The \ttt{-{}-out-earth boston\_offline.earth} option instructs the utility to
...
The \ttt{-{}-mt -{}-concurrency 8} arguments run the process in multithreaded
...
the \ttt{-h} switch for a brief usage help.

\begin{hint}
There is also a GUI frontend for \fprog{osgearth\_package} called
\fprog{osgearth\_package\_qt}. This tool provides an easy way to select the
...
location, an \cclass{osgEarth::GeoTransform} is very helpful. It takes
...
\cclass{osg::PositionAttitudeTransform} can be used to move and orient the
...
All annotations that can be created declaratively from an \ttt{.earth} file can also
be programmatically generated at runtime.

This example shows how the circular transmission ranges of the cows in the
osg-earth sample are created as a \cclass{osgEarth::Annotation::CircleNode}
annotation. Some basic styling is applied to it using an
\cclass{osgEarth::Style}, and the rendering technique is also specified.
