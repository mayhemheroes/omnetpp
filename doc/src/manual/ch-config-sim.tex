...
and \ttt{[Config Fifo2]}, each containing several \textit{entries}.
...
Long lines can be broken into multiple lines in two ways: using the traditional
...
**.app*.generationInterval = 3s # parameter value
...
file into logical units, fixed and varying parts, etc.
...
further includes other files, their path names will be understood as relative to
...
change the current section of the main file. The following example illustrates
the rules:
...
each configuration it is possible to specify the fallback section or a list of
...
object-oriented languages, and the benefits are similar too; one can factor out
...
Simulations receive input via module parameters, which can be assigned a
...
think of them as being ``hardcoded''. In contrast, it is easier
...
the top-level module and the \ttt{transactionsPerSecond} parameter of the
\ttt{server} module:
...
\textit{wildcard patterns} which allow setting several model parameters
...
patterns. The most apparent differences from globbing rules are the
distinction between \ttt{*} and \ttt{**}, and that character ranges should
be written with curly braces instead of square brackets, i.e.,
...
or \ttt{net.queue*.bufSize} selects only queues immediately on the network level.
...
enumerations of characters. For example, \ttt{\{\_a-zA-Z0-9\}} matches any letter
or digit, plus the underscore; \ttt{\{xyzc-f\}} matches any of the characters
x, y, z, c, d, e, f.
To include '-' in the set, put it in a position where it cannot be
interpreted as a character range, for example: \ttt{\{a-z-\}} or \ttt{\{-a-z\}}.
...
each host count three simulation runs will be conducted, with the generation
...
braces and other characters inside double-quoted string literals are
...
use backslash as an escape character, like in C/C++ and NED.
...
Here, the \ttt{foo} NED parameter will receive the following values in subsequent runs:
...
for the first value of \ttt{plan} the first values of \ttt{hosts} and \ttt{load}
...
\begin{note}
Remember that variables are substituted textually into the expression, so
...
Referencing an RNG number greater than or equal to this number
...
stream configuration -- even for simulation models that were not
...
The run number can be specified either in \ffilename{omnetpp.ini} (e.g. via the
...
For the now-obsolete \cclass{cLCG32} RNG, the name of the corresponding option is
...
each filter simultaneously. In addition, one can also specify a log prefix
format string which determines the context information that is written before
each log line. In the following sections, we look at how to configure logging.
...
are compiled into the executable. Any log statement which uses a log level below
...
simulation. By default, the runtime log level of all components is set to
...
In general, any log statement which uses a log level below the specified global
...
are the most generic runtime predicates that determine which log statements are
...
completely disable all logging by adding the following configuration option at the
