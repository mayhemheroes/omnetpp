...
NED files have the \ttt{.ned} file name suffix. This is mandatory, and
...
Simple module types are declared using the \fkeyword{simple} keyword;
...
parameters (\ref{sec:ned-ref:parameters}), and gates (\ref{sec:ned-ref:gates}).
...
a subclass of \cclass{cSimpleModule}. The way of associating the
...
Compound module types are declared using the \fkeyword{module} keyword;
see the NED Grammar (Appendix \ref{cha:ned-language-grammar}) for the
syntax.
...
\begin{note}
    A simple module can only be designated as a network by specifying the
    \fprop{@isNetwork} property; the \fkeyword{network} keyword may not
...
Channel types are declared using the \fkeyword{channel} keyword;
see the NED Grammar (Appendix \ref{cha:ned-language-grammar}) for the
syntax.
...
a subclass of \cclass{cChannel}. The way of associating the
NED type with the C++ class is described in section
\ref{sec:ned-ref:resolving-cpp-class}.
...
Module interface types are declared using the \fkeyword{moduleinterface} keyword;
see the NED Grammar (Appendix \ref{cha:ned-language-grammar}) for the
syntax.
...
Channel interface types are declared using the \fkeyword{channelinterface} keyword;
see the NED Grammar (Appendix \ref{cha:ned-language-grammar}) for the
syntax.
...
and channels, collectively referred to as \textit{components}), is identical.
It is as follows:
...
\begin{note}
    The \fprop{@class} property itself may contain a namespace declaration (i.e.
    may contain ``\ttt{::}'').
\end{note}

If the component contains no \fprop{@class} property and has no base class,
the C++ class name will be composed of the \textit{current namespace} and the
...
property found while searching in the following order:
...
\begin{note}
    ...
    in different scopes do not nest; rather, the nearest one wins.
\end{note}
...
The following example shows a property without an index, one with the index \ttt{index1},
...
If the key+equal sign part (\ttt{key=}) is missing, the \textit{valuelist} belongs
...
may have a value or default value and may also have properties
...
\fprop{@prompt}, \fprop{@mutable}, \fprop{@enum}, \fprop{@class}.
...
\subsubsection{The @enum Property}
\label{sec:ned-ref:enum-property}

The \fprop{@enum} property is only recognized for \fkeyword{string} parameters.
It defines a list of accepted values for the parameter. The values are specified
as a comma-separated list.

\begin{ned}
string protocol @enum("udp","tcp");
\end{ned}

At runtime, the simulation kernel will raise an error if the parameter is
assigned a value not in the list.


\subsubsection{The @class Property on Parameters}
\label{sec:ned-ref:class-param-property}

The \fprop{@class} property on an \fkeyword{object} parameter specifies the
expected C++ class of the parameter's value. When set, the simulation kernel
verifies that objects assigned to the parameter are instances of the given
class (or a subclass thereof).

\begin{ned}
object route @class(inet::L3Address);
\end{ned}

\begin{note}
    This use of \fprop{@class} is unrelated to the component-level \fprop{@class}
    property that specifies the C++ implementation class of a module or channel
    (see \ref{sec:ned-ref:resolving-cpp-class}).
\end{note}
...
\subsubsection{The @dynamic Property}
\label{sec:ned-ref:dynamic-property}

The \fprop{@dynamic} property is only recognized for submodules. The value may
be empty, \ttt{true} or \ttt{false}; \ttt{@dynamic} is equivalent to \ttt{@dynamic(true)}.

When a submodule is marked as dynamic inside a compound module, the submodule
will not be instantiated when the compound module is created; rather, it is
expected that it will be instantiated at runtime, using dynamic module creation.
A module created this way will pick up parameter values from the submodule
declaration in the NED file as well as from the ini file. Dynamic submodules
may be displayed in the graphical NED editor as semi-transparent, allowing them
to be edited and configured like other submodules.

\subsubsection{The @omittedTypename Property}
\label{sec:ned-ref:omittedtypename-property}

The \fprop{@omittedTypename} property is recognized on module interfaces,
channel interfaces, parametric submodules (\fkeyword{like}), and parametric
connections.  It simplifies the construction of highly configurable compound
modules where many submodules are optional, by reducing the need for complex
conditional connections.

When a parametric submodule's type name is configured as an empty string
(\ttt{""}), the network builder would normally raise an error. If the submodule
has an \fprop{@omittedTypename} property (which typically comes from the
interface), the specified type is instantiated instead. By convention, this type
is a lightweight \textit{self-destructing} module (e.g.\ \ttt{OmittedTcp}) that
shortcuts its input and output gates during initialization and then removes
itself from the compound module. This way, the surrounding connections remain
valid even though the submodule is effectively absent, without the compound
module needing complicated conditional connection logic.

\fprop{@omittedTypename} is typically specified on the module interface, so that
all submodules using that interface inherit the behavior:

\begin{ned}
moduleinterface ITCP {
    @omittedTypename(OmittedTcp);
}
\end{ned}

It may also be specified directly on a submodule or connection, which takes
precedence over the interface. If \fprop{@omittedTypename} is present but its
value is empty, the submodule will simply not be created (equivalent to being
conditionally omitted). For connections with parametric channel types, an empty
value means no channel object will be created for the connection.
...
\subsubsection{The @reconnect Property}
\label{sec:ned-ref:reconnect-property}

The \fprop{@reconnect} property is recognized for connections. Normally, it is
an error for a NED connection to refer to a gate that is already connected.
When \fprop{@reconnect} is present on a connection, the network builder first
disconnects any existing connections on the involved gates before creating the
new connection.
...
The primary use of \fprop{@reconnect} is in combination with inheritance,
where it allows a derived compound module to modify the connections established
by the base type, e.g., to insert a new submodule in the path between two
already connected submodules.

See also \ref{sec:ned-lang:reconnecting-gates}.
...
NED language expressions\index{ned!expressions} have a C-like syntax, with some
variations on operator names (see \ttt{\textasciicircum}, \ttt{\#}, \ttt{\#\#}).
...
of the model. Expressions can also use built-in and user-defined functions.
There is a JSON-like notation for defining arrays and objects (dictionary-like).

\begin{note}
New NED functions can be defined in C++; refer to section \ref{sec:sim-lib:defining-ned-functions}.
\end{note}

\subsection{Constants}
\label{sec:ned-ref:constants}

Refer to section \ref{sec:ned-ref:literals}.

\subsection{Array and Object Values}
\label{sec:ned-ref:arrays-and-objects}

A bracketed list of zero or more comma-separated expressions denotes an
\textit{array} value. For example: \ttt{[9.81, false, "Hello"]}.
...
Example 2 (includes class name): \ttt{Filter \{dest:"10.0.0.1", port:1200\}}.
...
The string match operator works as follows. \ttt{x =\ensuremath{\sim} pattern}
returns \ttt{true} if the string \ttt{x} matches the string \ttt{pattern}, and
\ttt{false} otherwise. The operator performs case-sensitive full-string match.
The pattern has the following syntax:

\begin{itemize}
  \item Asterisk \ttt{*} matches zero or more of any character except dot
  \item Double asterisk \ttt{**} matches zero or more characters (including dot)
  \item Curly braces containing a numeric range match an embedded whole number in that range
  \item Square brackets containing a numeric range match a number in that range
    enclosed in square brackets
  \item A numeric range has the syntax of \textit{<start>..<end>}, where both
    \textit{<start>} and \textit{<end>} are integers (optional)
\end{itemize}
...
Values may have the same types as NED parameters: boolean, integer, double,
string, XML element, and object. An integer or double value may have an
associated measurement unit (e.g., \ttt{s}, \ttt{mW}).

Double-to-integer conversions require explicit casting using the \ttt{int()}
function. There is no implicit conversion.

Integer-to-double conversion is implicit. However, a runtime error will be
raised if there is precision loss during the conversion, i.e., the integer is
too large to be precisely represented in a double. To suppress this error, an
explicit cast (\ttt{double()}) can be used.

There is no implicit conversion between boolean and numeric types. Thus, \ttt{0}
is not a synonym for \fkeyword{false}, and nonzero numbers are not a synonym for
\fkeyword{true}.

There is also no conversion between string and numeric types. For example,
\ttt{"foo"+5} is illegal. However, there are functions for converting a number
...
Addition, subtraction, and numeric comparisons require their arguments to have
the same unit or compatible units. If the units are compatible, a unit
conversion is performed before the operation. Incompatible units cause an error.
...
\begin{note}
If a floating-point modulo operator that handles units is needed, the
\ttt{fmod()} function can be used.
\end{note}

Multiplying two numbers with units is not supported.

For division, dividing two numbers with units is only supported if the two units
are convertible (i.e., the result will be dimensionless). Dividing a
dimensionless number with a number with a unit is not supported.

Operations involving quantities with logarithmic units (e.g., dB, dBW) are not
supported, except for comparisons. The reason is that such operations would be
easy to misinterpret. For example, it is not obvious whether \ttt{10dB+10dB}
(3.16+3.16) should evaluate to 20dB (=10.0) or to 16.02dB (=2*3.16=6.32),
considering that such quantities would often be hidden behind parameter names
where the unit is not obvious.


\subsection{Referencing Parameters, Gates and Loop Variables}
\label{sec:ned-ref:referencing-parameters-and-loop-variables}

Expressions may refer to parameters, gates, and other submodules using the
\fkeyword{this} or the \fkeyword{parent} qualifier, or without a qualifier.
Unqualified identifiers refer to the parameters, gates or submodules of the
(nearest) block in which they are defined: the submodule or the module/channel.
In connection \fkeyword{for} loops, if an identifier matches a previously
defined loop variable, it refers to that loop variable.
Namely:\footnote{The scoping rules for unqualified identifiers inside submodule
and connection blocks changed in {\opp} 6.0. In 5.x, unqualified names resolved
to the enclosing compound module's items; since 6.0, they resolve to the local
submodule or channel's items. To write NED compatible with both versions, qualify
all references within submodule and connection blocks with explicit
\fkeyword{parent} or \fkeyword{this}, and require {\opp} 5.7 or later.}

\begin{description}
  \item[Outside submodule and connection blocks:] Unqualified identifiers refer
    to parameters, gates, and submodules of the compound module being defined.
  \item[Inside submodule and connection blocks:] Unqualified identifiers refer
    to items of (or within) the \textit{local} submodule or channel object, not
    to items of the enclosing compound module. To refer to the compound module's
    items from within a submodule or connection block, use the
    \fkeyword{parent} qualifier. To make references to the local submodule or
    channel explicit, use the \fkeyword{this} qualifier.
\end{description}

Example demonstrating the semantics:
...
network Network {
  ...
  submodules:
    ...
    node2: Node {
      ...
      bar = this.foo;  // OK: refers to node2.foo
      baz = node1.foo;  // ERROR: refers to yet-uncreated submodule this.node1!
      bax = parent.node1.foo;  // OK: refers to Network.node1.foo
    }
}
\end{ned}

Expressions may also refer to parameters of submodules defined earlier in the
NED file using the \ttt{parent.submoduleName.paramName} or
\ttt{parent.submoduleName[index].paramName} syntax from within a submodule block.

\begin{note}
Referencing a submodule's submodule from NED (i.e., the \ttt{a.b} or
\ttt{this.a.b} syntax within a submodule block) is generally wrong, because the
network is built top-down and submodules are only created after parameters have
already been set up. An exception is using such references in the values of
\fkeyword{volatile} parameters, because they are evaluated later, at access time.
\end{note}
...
The \fkeyword{typename} operator returns the NED type name as a string. If it
occurs inside a component definition but outside a submodule or channel block,
it returns the type name of the component being defined. If it occurs inside a
submodule or channel block, it returns the type name of that submodule or
channel.

The \fkeyword{typename} operator can also occur in the \fkeyword{if} condition
of a scalar submodule or connection. In such cases, it evaluates to the
\textit{would-be} type name of the submodule or condition. This allows for
conditional instantiation of parametric-type submodules, controlled from a
...
The \fkeyword{typename} operator is not allowed in a submodule vector's
\fkeyword{if} condition. The reason is that the condition applies to the vector
as a whole while the type is per-element.

\subsection{The \fkeyword{index} Operator}
\label{sec:ned-ref:index-operator}

The \fkeyword{index} operator is only allowed in a vector submodule's body and
yields the index of the submodule instance.

\subsection{The \fkeyword{exists()} Operator}
\label{sec:ned-ref:exists-operator}

The \fkeyword{exists()} operator is only accepted in compound module definitions.
It returns \fkeyword{true} if the specified item exists, and \fkeyword{false}
otherwise.

The operator accepts various forms of arguments:

\begin{itemize}
  \item \ttt{exists(submoduleName)} -- checks if a (typically conditional) submodule exists
  \item \ttt{exists(submoduleName[index])} -- checks if an element in a submodule vector exists
        (useful with submodule vectors that may contain holes due to \ttt{@omittedTypename})
  \item \ttt{exists(parent.submoduleName)} -- explicitly refers to the parent module's submodule
  \item \ttt{exists(this.submoduleName)} -- checks for a submodule within the current submodule
        (generally only useful in \fkeyword{volatile} parameter expressions)
\end{itemize}
...
The \fkeyword{sizeof()} operator expects one argument and is only accepted in
compound module definitions. It returns the size of a submodule vector or gate
vector. The usual scoping rules apply to the argument (see
\ref{sec:ned-ref:referencing-parameters-and-loop-variables}): use the
\fkeyword{this} and \fkeyword{parent} qualifiers to disambiguate.

In particular, to refer to the size of a sibling submodule's gate vector, use
the \ttt{sizeof(parent.submoduleName.gateVectorName)} or
\ttt{sizeof(parent.submoduleName[index].gateVectorName)} syntax from within a
submodule block.
...
expression as an argument and returns an object that encapsulates the expression
...
Identifiers in the expression are not interpreted as parameter references as in
...
The XPath expression is used to select an element within the document. If the
expression matches several elements, the first element (in preorder depth-first
traversal) will be selected (unlike XPath, which selects all matching nodes).

The expression syntax is as follows:
\begin{itemize}
  \item An expression consists of \textit{path components} (or "steps")
    separated by "\ttt{/}" or "\ttt{//}".
  \item A path component can be an element tag name, "\ttt{*}", "\ttt{.}", or
    "\ttt{..}".
  \item "\ttt{/}" means child element (just like in \ttt{/usr/bin/gcc}), while
    "\ttt{//}" means an element at any level under the current element.
  \item "\ttt{.}", "\ttt{..}", and "\ttt{*}" mean the current element, the
    parent element, and an element with any tag name, respectively.
  \item Element tag names and "\ttt{*}" can have an optional predicate in the
    form "\ttt{[position]}" or "\ttt{[@attribute='value']}". Positions start
    from zero.
  \item Predicates of the form "\ttt{[@attribute=\textit{\$param}]}" are also
    accepted, where \ttt{\textit{\$param}} can be one of the following items:
...
The following measurement units are recognized in constants. Other units can be
used as well, but there are no conversions available for them (i.e.,
...
d & day & 86400s \\\hline
  ...
  B & byte & 8b \\\hline
  KiB & kibibyte & 8192b \\\hline
  MiB & mebibyte & 8.38861e6b \\\hline
  GiB & gibibyte & 8.58993e9b \\\hline
  TiB & tebibyte & 8.79609e12b \\\hline
  kB & kilobyte & 8000b \\\hline
  MB & megabyte & 8e6b \\\hline
  GB & gigabyte & 8e9b \\\hline
  TB & terabyte & 8e12b \\\hline
  b & bit & \\\hline
