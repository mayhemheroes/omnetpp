...
As mentioned earlier, software testing techniques can significantly aid in model
...
Subsequent sections will introduce \fprog{opp\_test}, a tool provided by {\opp}
...
This section documents \fprog{opp\_test}, a versatile tool that is
...
\fprog{opp\_test} is built around a simple concept: it allows the user to define
...
Let's see a small test file, \ttt{cMessage\_properties\_1.test}:
...
Note that code from all tests was linked to form a single executable, which saves
...
\item \textit{test program}: The simulation program whose output is checked by the test.
  ...
  then use e.g. \fprog{opp\_run} as the test program.
\item \textit{test directory}: The directory where a \ttt{.test} file
  is extracted; usually \ttt{work/<testname>/}. It is also set as the working
...
and \ftest{\%global} blocks, but for other files (e.g., source files generated
...
the test program; for the latter two, \ttt{stdout} and \ttt{stderr} need to be
specified as file names, respectively. If the file is not found, the test will be marked
as an \textit{error}. There can be several \ftest{\%contains}-style directives
in the test file.
...
matched against expected output. This is done with the \ftest{\%subst}
...
\subsubsection{\%round-numbers}
\label{sec:testing:opptest:round-numbers}

Syntax:

\begin{filelisting}
%round-numbers: <significant-digits>
\end{filelisting}

This directive rounds all floating-point numbers in the output to the
specified number of significant digits before matching against expected
output. It is applied before \ftest{\%subst} rules. The rounding uses
\textit{\%.<N>g} formatting, which strips trailing zeros.

This is useful for eliminating floating-point representation noise that
may appear when output precision is high. For example, a bin edge that is
mathematically 0.06 may be stored as the IEEE 754 double
0.059999999999999998. At high output precision, this noise becomes visible,
and poses a challenge when specifying the expected output.
With \ftest{\%round-numbers}, the value is first properly
rounded, so subsequent truncation produces the correct result.
...
Little has been said so far about what \fprog{opp\_test} actually does or how it
...
\fprog{opp\_makemake}, and the last one can be done by setting and exporting
...
Smoke tests are a tool for basic verification and regression testing.
...
on the command line (\ttt{-\-fingerprint=...}) will enable fingerprint
...
Ingredients may also be specified with the \fconfig{fingerprint-ingredients}
configuration option. However, that is rarely necessary because the ingredients
list included in the fingerprints takes precedence and is also more convenient
to use.
...
\begin{itemize}[noitemsep]
  ...
  \item[\tbf{b}]: message contents*
  \item[\tbf{g}]: message control info*
  \item[\tbf{d}]: message data (legacy)*
  \item[\tbf{i}]: module id
...
Most fingerprint ingredients require no explanation, but the ones marked with
asterisk above do:

\begin{itemize}
  \item[\tbf{b}] \textbf{message contents}: Incorporates the core message data,
    excluding control info and technical bookkeeping fields from the last
    transmission. This ingredient is insensitive to simulation time and the
    identity of sender/receiver modules. Uses \ffunc{parsimPack()} to serialize
    objects into byte arrays for fingerprint inclusion.

  \item[\tbf{g}] \textbf{message control info}: Includes control information
    objects attached to arriving messages. Uses \ffunc{parsimPack()} to serialize
    objects into byte arrays for fingerprint inclusion.

  \item[\tbf{d}] \textbf{message data}: Legacy ingredient for message data
    inclusion. Uses \ffunc{parsimPack()} to serialize objects into byte arrays
    for fingerprint inclusion. This is retained for backward compatibility with
    older fingerprints.

  \item[\tbf{x}] \textbf{extra data added programmatically}: Includes data
    explicitly added to the fingerprint via \ffunc{addExtraData()} method calls in
    the simulation code. This allows custom data to be incorporated into
    fingerprint computation.

  \item[\tbf{f}] \textbf{canvas figures}: Includes information about graphical
    figures displayed on the simulation canvas. Primarily incorporates geometry
    information (positions, sizes, shapes) but excludes styling details (colors,
    fonts, line styles). This design choice makes fingerprints more practical for
    regression testing, as geometry typically carries important functional
    information while styling is often a matter of visual preference.
\end{itemize}

These ingredients enable more detailed fingerprint-based regression tests and can help detect subtle differences in message traffic and simulation behavior that might not be captured by other fingerprint ingredients.
...
tool (the \ftest{\%activity} directive is especially useful here), or with
...
% TODO explain more, with a concrete example for the control script?
...
tool. A \ttt{.test} file allows one to place the test description, test setup,
...
Statistical software like \textit{GNU R} is extremely useful for performing
these tests.

Statistical tests can also be implemented in \ttt{.test} files. To run several
simulations within one test, use \ftest{\%extraargs} to pass the \fopt{-r
<runs>} option to Cmdenv; alternatively, \ftest{\%testprog} can be used to have
the test tool run \fprog{opp\_runall} instead of the normal simulation program.
For performing statistical computations, use \ftest{\%postrun-command} to run a
Python or R script. The Python script may rely on the Pandas, SciPy, and \ttt{omnetpp.scave}
