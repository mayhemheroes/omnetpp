...
that \ffunc{activity()} does not encourage good programming style, and
stack switching can confuse many debuggers.

There is one scenario where \ffunc{activity()}'s process-style
...
There are situations when you certainly do not want to use \ffunc{activity()}.
If the \ffunc{activity()} function contains no \ffunc{wait()} and has only one
\ffunc{receive()} at the top of a message handling loop, there is no point in
using \ffunc{activity()}, and the code should be written with \ffunc{handleMessage()}.
...
For example, a \ffunc{Sink} implementation:
...
\ffunc{activity()} runs as a coroutine\index{coroutine}.
...
\ffunc{transferTo()} involves switching from one CPU stack to
another.
...
booted by events (starter messages\index{starter messages})
...
that request events from the simulation kernel are \ffunc{receive()} and
\ffunc{wait()}, so their
...
You do need \ffunc{finish()} if you want to write statistics at
...
\begin{itemize}
   ...
   \item does not encourage good programming style: as module complexity
       grows, \ttt{activity()} tends to become a large, monolithic function.
\end{itemize}
...
%
% Coroutines are used by several other simulation packages:
% \begin{itemize}
% \item All simulation software that inherits from SIMULA (e.g. C++SIM)
%     is based on coroutines, although the programming
%     model is quite different overall.
% \item The simulation/parallel programming language Maisie and its successor
%     PARSEC (from UCLA) also use coroutines (though they are implemented
%     with ``normal'' preemptive threads). The philosophy
%     is quite similar to {\opp}. PARSEC, being ``just''
%     a programming language, has a more elegant syntax but far fewer
%     features than {\opp}.
...
class at runtime and can be accessed\index{module!accessing parameters} by
calling the \ffunc{par()} member function of \cclass{cComponent}:
...
The value in a \cclass{cPar} object can be read with methods that correspond to
the parameter's NED type: \ffunc{boolValue()}, \ffunc{intValue()},
\ffunc{doubleValue()}, \ffunc{stringValue()}/\ffunc{stdstringValue()},
\ffunc{objectValue()}, \ffunc{xmlValue()}. There are also overloaded typecast
operators for the corresponding types (\ttt{bool}, integer types including
\ttt{int} and \ttt{long}, \ttt{double}, \ttt{const char *}, \ttt{cObject*}, and
\ttt{cXMLElement*}).
...
\ffunc{stdstringValue()}, which returns \ttt{std::string}. For volatile
parameters, only \ffunc{stdstringValue()} may be used, but otherwise the two are
interchangeable.

If you use the \ttt{par("foo")} parameter in expressions (such as
\ttt{4*par("foo")+2}), the C++ compiler may be unable to decide between
overloaded operators and report ambiguity. This issue can be resolved by adding
an explicit cast such as \ttt{(double)par("foo")}, or using the
\ffunc{doubleValue()} or \ffunc{intValue()} methods.
...
Parameters can be declared volatile by marking them with the \fkeyword{volatile}
...
\ffunc{initialize()} function and storing that value for subsequent use is
incorrect.

Volatile parameters are often used to allow stochastic input, such as random
packet generation intervals specified e.g. as \ttt{exponential(1.0)} (numbers
...
parameter is assigned an expression like \ttt{exponential(1.0)}, multiple reads
will yield the same randomly chosen value.

The typical usage for non-volatile parameters is to read them in the
\ffunc{initialize()} method of the module class and store the values in class
...
\ttt{volatile} parameters need to be re-read every time the value is needed. For
...
Parameters declared with the type \fkeyword{object} in NED can be accessed with
the \ttt{objectValue()} method of \cclass{cPar}. It returns a pointer of the
type \ttt{cObject*}, which then must be cast to the appropriate type using
\ttt{check\_and\_cast()} or \ttt{dynamic\_cast()}.
...
the \cclass{cValueArray} class, and dictionaries represented by the
\cclass{cValueMap} class. Values inside \cclass{cValueArray} and
\cclass{cValueMap} are stored in \cclass{cValue} instances. When a single value
is assigned to an object parameter, it is represented as a \cclass{cValue}
wrapped in a \cclass{cValueHolder}. \footnote{Object parameters were introduced
in version 6.0 of {\opp}. In earlier versions, string parameters were often used
to emulate arrays and were parsed with the help of the \cclass{cStringTokenizer}
class and its convenience methods, such as \ffunc{asIntVector()} and
\ffunc{asDoubleVector()}.}

To query and process JSON-style parameters in your module's C++ code,
particularly within the \ttt{initialize()} method, you would retrieve the object
using the \ttt{par()} and \ttt{objectValue()} methods, cast them to the
...
objects, each specifying \ttt{dest}, \ttt{netmask}, \ttt{interf}, and
\ttt{metric}.
...
In the module's \ffunc{initialize()} method, you can process this parameter as follows:
...
This example demonstrates how to access a JSON-style parameter (\ttt{routes}),
iterate over its elements (routes), and extract and use the data in the
simulation module's logic. The use of \cclass{cValueMap} and
\cclass{cValueArray} classes makes handling structured data straightforward,
...
\ffunc{setDoubleValue()}, \ffunc{setObjectValue()}, \ffunc{setXMLValue()}.
There are also overloaded assignment operators for various types including
\ttt{bool}, \ttt{int}, \ttt{long}, \ttt{double}, \ttt{const char *},
\ttt{cObject*}, and \ttt{cXMLElement*}.
...
\ffunc{getType()} methods. The latter returns a value from an enum that can be
converted to a readable string with the \ffunc{getTypeName()} static method. The
enum values are \ttt{BOOL}, \ttt{DOUBLE}, \ttt{INT}, \ttt{STRING}, \ttt{OBJECT},
and \ttt{XML}, and since the enum is an inner type, they usually have to be
qualified with \ttt{cPar::}.
...
\begin{note}
  When implementing a \ffunc{handleParameterChange()} method, care must be taken
  to handle the changes of all parameters marked as \fprop{@mutable} in the NED
  file.
\end{note}
...
\ffunc{handleParameterChange()} methods need to be implemented carefully because
...
\ffunc{handleParameterChange()}, as it is easy to accidentally create an
...
The \cclass{cModule} class has several member functions that
...
error. The \ffunc{gate()} method needs to be told whether you need the input or
output half of the gate. This can be done by appending
\ttt{"\$i"} or \ttt{"\$o"} to the gate name. The following example
...
the name of the inout gate and either \ttt{cGate::INPUT} or \ttt{cGate::OUTPUT}:
...
For that purpose, there is a \ffunc{hasGate()} method. For example:
...
The \ffunc{gate()} method also has an overloaded variant that
...
Gate vectors have one \cclass{cGate} object per element.
...
The size of the gate vector can be obtained using the \ffunc{gateSize()}
...
A gate vector cannot have ``holes'' in it, which means that \ffunc{gate()}
...
The \ffunc{hasGate()} method can be used both with and without an
...
Two other important properties of gate IDs are that they are \textit{stable}
...
two possibilities. One is to use the \ffunc{getGateNames()} method,
...
Here, \ttt{this} denotes the module whose gates are being enumerated
...
\begin{note}
    ...
    Additionally, the \ffunc{gate()} method now throws an error when called
    with an invalid ID, rather than returning \ttt{nullptr}.
\end{note}
...
remove one half of an inout gate (e.g. \ttt{"gate\$o"}), or set
...
Gate vector size can be changed using \ffunc[setGateSize()]{setGateSize(name,size)}.
None of these methods accept a \ttt{"\$i"} or \ttt{"\$o"} suffix in gate names.
...
as well, use \ffunc{getFullName()}. If you also want to
...
either \fvar{cGate::INPUT} or \fvar{cGate::OUTPUT}. (It cannot return
\fvar{cGate::INOUT} because an inout gate is represented by a pair
of \cclass{cGate}s.)
...
\ffunc{getNameSuffix()} methods. The \ffunc{getBaseName()} method returns the
...
The methods \ffunc{isVector()}, \ffunc{getIndex()}, \ffunc{getVectorSize()} speak
for themselves; \ffunc{size()} is an alias for \ffunc{getVectorSize()}. For
...
The \ffunc{getOwnerModule()} method returns the module to which the gate object
belongs.
...
and \ffunc{getNextGate()} methods). Therefore, a path can be thought of as
...
The start and end gates of the path can be found using the \ffunc{getPathStartGate()}
...
The \ffunc{isConnected()} method is a bit different: it returns \ttt{true} if the gate
...
The following code prints the name of the gate to which a simple module gate is
connected:
...
The result may be \ttt{nullptr}, meaning that a connection may not have
an associated channel object.

If you have a channel pointer, you can get the source gate of the channel
using the \ffunc{getSourceGate()} method:
...
\ffunc{isTransmissionChannel()} method returns \ttt{true}. However,
\cclass{cGate} has a convenience method for this called
\ffunc{getTransmissionChannel()}. Here is an example usage:
...
is not desirable, you can use the similar \ffunc{findTransmissionChannel()} and
\ffunc{findIncomingTransmissionChannel()} methods, which simply return
...
its subclasses. Network packets are represented by \cclass{cPacket},
which is also a subclass of \cclass{cMessage}. Message objects are
...
\ttt{cMessage *} pointers. In the examples below, messages will be
created with \ttt{new cMessage("foo")}, where
...
the module can call the \ffunc{isSelfMessage()} member of any received
message to determine if it is a self-message.

You can determine whether a message is currently in the FES by calling
its \ffunc{isScheduled()} member function.

\subsubsection{Canceling an Event}
\label{sec:simple-modules:canceling}

Scheduled self-messages can be canceled\index{self-message!canceling}
...
you may delete the message or reuse it in subsequent \ffunc{scheduleAt()}
calls. \ffunc{cancelEvent()} has no effect if the message is not
...
imaginary stop-and-wait protocol. The code uses a \ttt{timeoutEvent}
...
simulation time, it must first be canceled using \ffunc{cancelEvent()}.
...
call, using the functions \ffunc{rescheduleAt()} and \ffunc{rescheduleAfter()}.
...
\ffunc{cancelEvent()} + \ffunc{scheduleAt()} combination.

\subsection{Sending Messages}
\label{sec:simple-modules:sending-messages}

Once created, a message object can be sent through an output
gate using one of the overloaded \ffunc{send()} methods of \cclass{cSimpleModule}.
There are six variations available, as the gate can be specified in
multiple ways and the methods also accept an optional \cclass{SendOptions} structure:
...
The most common way of specifying the gate is with its name (\ttt{gateName}
parameter). If the name identifies a gate vector, an additional \ttt{gateIndex}
...
components of an inout gate are distinguished by appending the \ttt{\$i} and
\ttt{\$o} suffixes to their names, respectively. Thus, the gate name needs to be
...
Using a gate pointer (\ttt{cGate*}) will result in more efficient code, as it
spares the lookup inside the \ffunc{send()} call. Typically, the module code
...
Using a gate ID (\ttt{gateId} parameter) is slightly less efficient than using
...
The optional \cclass{SendOptions}, as well as other \textit{send} variants like
\ttt{sendDelayed()} and \ttt{sendDirect()}, will be covered in later sections.

\subsubsection{What send() Does}
\label{sec:simple-modules:what-send-does}

The \ffunc{send()} call causes the message to travel along the full length of
...
the series of connections defined by the \ffunc{getNextGate()} method of
\cclass{cGate}; the path ends when \ffunc{getNextGate()} returns \ttt{nullptr}.
...
\ffunc{processMessage()} method of \cclass{cChannel} is invoked with the message
as an argument (and with some extra arguments such as \cclass{SendOptions}).
Individual channel types override the \ffunc{processMessage()} method to apply
...
After the message has reached the last gate in the connection path (the gate
where \ffunc{getNextGate()} returns \ttt{nullptr}), the message will be passed
to the \ffunc{arrived()} method of the module to which the last gate belongs.
By default, the \ffunc{arrived()} method inserts the message into the FES,
scheduled for the message's arrival time, before returning. The message will
only be actually passed to the module's \ffunc{handleMessage()} (or \ffunc{activity()})
method when the simulation has advanced to the point where the message becomes
the first event in the FES.

The \ffunc{arrived()} method is not normally overridden in simulation models.
However, it is noteworthy that the implementation of \ffunc{arrived()} in
\cclass{cModule} (which commonly represents compound modules) stops the
...
will be generated: \textit{not owner of message}.
  ...
  it uses object ownership\index{ownership} management (described in
...
and then send it. In such cases, you can use the \ffunc{scheduleAt()} function,
...
These methods are similar to the regular \ffunc{send()} methods, but with an
...
The \ffunc{sendDelayed()} function does not perform a \ffunc{scheduleAt()}
followed by a \ffunc{send()}, but rather it computes everything about the
message sending up front, including the arrival time and the target module.
This has two consequences. First, \ffunc{sendDelayed()} is more efficient than
a \ffunc{scheduleAt()} followed by a \ffunc{send()} because it eliminates one
...
\begin{note}
  Although \ffunc{sendDelayed()} is more efficient, you should think twice
...
The \ffunc{sendDirect()} function allows for sending a message directly to an
input gate of another module. This is useful for simulating wireless
transmissions. \ffunc{sendDirect()} has several variants because the target gate
...
\cclass{SendOptions} structure.

Here are the variants of \ffunc{sendDirect()}:
...
have dedicated gates to be able to receive messages sent via \ffunc{sendDirect()}.
It is not possible to have a gate that receives messages via both connections
and \ffunc{sendDirect()}.
...
packet, i.e. subclassed from \cclass{cPacket}. In that case, the duration
will be written into the packet, and can be read by the receiver with the
\ffunc{getDuration()} method of the packet. For non-packet messages, the
...
send another packet. (This applies to the default behavior of
\cclass{cDatarateChannel}; see section
\ref{sec:simple-modules:dataratechannel-modes} for alternatives that
...
\begin{note}
  ...
  subtracted from the value returned by \ffunc{getTransmissionFinishTime()}!
...
{\omnetpp} allows for modeling such scenarios using a special mechanism called
...
version 6.0, using the \ffunc{forceTransmissionFinishTime()} channel method was
...
(\ffunc{send()}, \ffunc{sendDirect()}, etc.), specifying a \cclass{SendOptions}
that has the \ttt{updateTx} field set and contains the transmission ID.
It is recommended to use utility functions of \cclass{SendOptions}, such as
\ffunc{finishTx()} and \ffunc{updateTx()}, to set up the \cclass{SendOptions}.
...
\begin{itemize}
  ...
  \item If the channel does not contain the data rate, the sender must specify
    it explicitly in \cclass{SendOptions}.
  ...
  \item Or if the channel does not keep track of the transmission start times,
    the remaining duration must be specified by the model in \cclass{SendOptions}.
  \item Etc.
\end{itemize}

The \cclass{cDatarateChannel} class, the default transmission channel type in {\omnetpp},
supports many variations of the above.

For wireless transmissions modeled with \ffunc{sendDirect}, there is no channel,
...
and fields, it is necessary to cast the incoming message to \cclass{cPacket}.

\cclass{cPacket} has several fields that provide information about the packet's
last transmission over the transmission channel. These fields are:

\begin{itemize}
  \item \ffunc{hasBitError()}: Returns \ttt{true} if the packet has any bit errors.
  \item \ffunc{isUpdate()}: Returns \ttt{true} if the packet is a transmission update.
  \item \ffunc{getTransmissionId()}: For transmission updates, returns the ID that identifies the transmission.
  \item \ffunc{isReceptionStart()}: Returns \ttt{true} if the packet represents the start of the reception process.
  \item \ffunc{isReceptionEnd()}: Returns \ttt{true} if the packet represents the end of the reception process.
  \item \ffunc{getDuration()}: Retrieves the total transmission duration of the packet.
  \item \ffunc{getRemainingDuration()}: Obtains the remaining transmission time of the packet.
\end{itemize}
...
\subsubsection{Checking for Bit Errors}
...
receiver's responsibility to check this flag using \ffunc{hasBitError()} and
...
behavior, call \ttt{gate("in")->setDeliverImmediately(true);} in the module's
\ffunc{initialize()} method. This setting causes packets to be delivered at the
start of reception.
...
The \ffunc{setDeliverImmediately()} method only needs to be called once,
so it is typically done in the \ffunc{initialize()} method of the module.
...
\ffunc{isReceptionStart()} and \ffunc{isReceptionEnd()} methods to
determine whether it represents the start or end of the reception process.
(Note that for a transmission update, both methods may return \ttt{false}.)


\subsubsection{Handling Transmission Updates}
\label{sec:simple-modules:reception-of-transmission-updates}

The receiver should recognize transmission updates using \ttt{isUpdate()} and
react accordingly.
...
\ttt{setDeliverImmediately(true)} in the previous section) should be prepared
...
and reschedule any potential end-reception event to \ttt{simTime() + pk->getRemainingDuration()}.
...
by calling \ttt{setTxUpdateSupport(true)}, usually in the \ffunc{initialize()} method.
...
Modules based on \ffunc{activity()} receive messages using the \ffunc{receive()}
method of \cclass{cSimpleModule}. The \ffunc{receive()} method cannot be used with
modules based on \ffunc{handleMessage()}.
...
parameter\index{receive!timeout} (in the form of a \textit{delta}, not an
absolute simulation time). If no message arrives within the timeout period,
the function returns \ttt{nullptr}.
...
The \ffunc{wait()} function suspends the execution of the module for a given
amount of simulation time (a \textit{delta}). \ffunc{wait()} cannot be used with
modules based on \ffunc{handleMessage()}.
...
Internally, the \ffunc{wait()} function is implemented by a combination of
\ffunc{scheduleAt()} and \ffunc{receive()}. The \ffunc{wait()} function is very
...
of class \cclass{cQueue}, described in Chapter \ref{cha:sim-lib}, in addition
to the wait interval. Messages that arrive during the wait interval are
accumulated in the queue and can be processed after the \ffunc{waitAndEnqueue()}
call returns.
...
When subclassing \cclass{cChannel}, the following pure virtual member functions
...
\begin{itemize}
  ...
  \item \ffunc[processMessage()]{Result processMessage(cMessage *msg, const SendOptions\& options, simtime\_t t)}
\end{itemize}
...
\cclass{cChannel} also provides an \ffunc{isDisabled()} virtual method
(returning \ttt{false} by default) that subclasses can override.
Both \cclass{cDelayChannel} and \cclass{cDatarateChannel} override it
to return the value of their \ttt{disabled} NED parameter. A disabled
channel discards all messages sent on it.
...
The \cclass{Result} struct is an inner type of \cclass{cChannel}
and looks like this:
...
\ffunc{isPacket()} returns true); they should be skipped for non-packet
...
The \ffunc{processMessage()} method does not need to throw an error on overlapping
...
% intended for use as channels should implement the \ttt{ned.IBidirectionalChannel}
...
\cclass{cIdealChannel} lets messages and packets pass through without
...
sets the appropriate fields in the \cclass{Result} struct:
...
\cclass{cDatarateChannel} is different. It performs packet duration modeling
(duration is calculated from the data rate and the length of the packet),
so \ffunc{isTransmissionChannel()} returns \ttt{true}.
\ffunc{getTransmissionFinishTime()} returns the value of the
\ttt{channelFinishTime} data member, which gets updated after every packet.
...
An important detail is that non-packet messages (such as \cclass{cMessage}
instances) pass through the channel without interfering with packet
transmissions. They are only subject to the propagation delay, and are
discarded if the channel is disabled.
                    ...
                    const SendOptions& options, simtime_t t)
{
    ...
    if (msg->isPacket()) {
        // packet transmission: disabled check, duration, error modeling
        processPacket(static_cast<cPacket *>(msg), options, t, result);
    }
    else {
        // non-packet messages: only subject to disabled check
        if (isDisabled)
            result.discard = true;
    }
...
The \ffunc{processPacket()} helper function (not shown in full) handles
computing the transmission duration, either from the packet's bit length
and the channel's data rate, or from the explicit duration specified in
\cclass{SendOptions}; bookkeeping of the transmission state including
support for transmission updates; and bit error modeling.\footnote{The
actual code is more complex than shown here. Refer to the source file
\ttt{cdataratechannel.cc} for the full implementation.}


\subsection{cDatarateChannel Operating Modes}
\label{sec:simple-modules:dataratechannel-modes}

\cclass{cDatarateChannel} supports three operating modes, selectable via the
\ffunc{setMode()} method. The mode determines how the channel handles
overlapping transmissions, and affects the behavior of the
\ffunc{isBusy()} and \ffunc{getTransmissionFinishTime()} methods.

\begin{itemize}
  \item \ttt{SINGLE} (the default): At most one transmission can be active at
    any given time. If a module attempts to send a packet while the channel is
    still busy with a previous transmission, a runtime error is raised. This
    is the traditional behavior, and it is appropriate for simple point-to-point
    links. The \ffunc{isBusy()} and \ffunc{getTransmissionFinishTime()} methods
    report the state of the single ongoing transmission.

  \item \ttt{MULTI}: Multiple concurrent transmissions are allowed, and the
    channel tracks all of them individually. This is useful for modeling a channel
    with multiple subchannels or carriers. Transmission updates must refer to an
    existing and still active transmission via the transmission ID.
    The \ffunc{isBusy()} method returns \ttt{true} if any of the tracked
    transmissions is still in progress, and \ffunc{getTransmissionFinishTime()}
    returns the latest finish time among all active transmissions.

  \item \ttt{UNCHECKED}: Multiple concurrent transmissions are allowed, but
    the channel does not keep track of ongoing transmissions. This is the
    low-overhead version of \ttt{MULTI}. As a consequence, no consistency
    checks are performed, the \ffunc{isBusy()} and
    \ffunc{getTransmissionFinishTime()} methods are not supported (they throw
    an error if called), and the channel does not emit \ttt{channelBusy}
    signals. In this mode, both \ttt{duration} and \ttt{remainingDuration}
    must be explicitly specified in \cclass{SendOptions} for transmission updates,
    because the channel has no state to compute the missing value from.
\end{itemize}

The mode can be set programmatically using \ffunc{setMode()}:

\begin{cpp}
cDatarateChannel *ch = check_and_cast<cDatarateChannel *>(
    gate("out")->getTransmissionChannel());
ch->setMode(cDatarateChannel::MULTI);
\end{cpp}

\begin{note}
  The mode should be set before the channel starts being used for transmissions.
  Switching the operating mode while transmissions are in progress is not
  allowed and will result in a runtime error.
\end{note}


\subsection{Optional Data Rate in cDatarateChannel}
\label{sec:simple-modules:optional-datarate}

The data rate of a \cclass{cDatarateChannel} is optional: it can be set to
\ttt{0} or \ttt{nan} (in NED) to leave it unspecified. When the data rate is
unspecified, the channel cannot compute the transmission duration from the
packet length, so the sender must explicitly provide it in the send call
via \cclass{SendOptions}:

\begin{cpp}
simtime_t duration = pkt->getBitLength() / selectedBitrate;
send(pkt, SendOptions().duration(duration), "out");
\end{cpp}

This feature is useful for modeling links where the data rate can vary on a
per-packet basis, such as wireless links with adaptive modulation and coding
schemes, or multi-rate Ethernet.

If neither the channel data rate nor an explicit duration is specified, a
...
You can terminate the simulation using the \ffunc{endSimulation()} function:
...
Do not include a newline (\ttt{{\textbackslash}n}), period, or exclamation mark
...
Finite State Machines\index{finite state machine} (FSMs)
can make life easier when dealing with \ffunc{handleMessage()}. {\opp} provides a
...
\begin{itemize}
  \item There are two kinds of states: \textit{transient} and \textit{steady}. On
    each event (that is, at each call to \ffunc{handleMessage()}), the FSM
...
State transitions are done via calls to \fmac{FSM\_Goto()}, which simply stores
the new state in the \cclass{cFSM} object:
...
FSMs perform their logging via the \fmac{FSM\_Print()} macro, defined as something like this:
...
inclusion of \ffilename{omnetpp.h}, and providing a new definition.
...
The path is a dot-separated list of module names. The special module name,
...
The following lines demonstrate relative paths. They find the \ttt{app[3]}
...
\cclass{cModule} can be used to obtain a \ttt{cModule*} pointer to the
...
Using \ttt{check\_and\_cast} saves you from writing error checking code:
if \ttt{targetModule} from the first line is \ttt{nullptr} because
...
accepts \ttt{nullptr} as input and only complains if the cast goes wrong.}
...
to \fmac{Enter\_Method()}. Both macros accept a \ttt{printf()}-like
...
As you already know, simple modules with \ffunc{activity()} need a
...
\ffunc{preDelete()} receives an argument: the pointer of the module on which
...
which is an optional argument of \ffunc{emit()}.
...
a parameter and returns the corresponding \ttt{simsignal\_t} value.
...
a \ttt{const char *} (or \ttt{nullptr} for an invalid signal handle):
...
The former returns a \ttt{std::vector<simsignal\_t>}; the latter takes
a signal ID (\ttt{simsignal\_t}) and returns a \ttt{std::vector<cIListener*>}.
...
methods with bodies that throw a \ttt{"Data type not supported"} error.
...
\begin{itemize}
  ...
  \item \cclass{cPostModuleBuildNotification}
  \item \cclass{cPostComponentInitializeNotification}
...
\fprop{@statistic} properties to the NED definition of the simple module
or channel. A \fprop{@statistic} property defines the name of the
...
\ref{sec:ned-lang:properties}). The property name is always \ttt{statistic},
and the index (here, \ttt{queueLength}) is the name of the statistic. The
property value, that is, everything inside the parentheses, provides hints
and extra information for recording.

The above \ttt{@statistic} declaration assumes that the module's C++ code
...
\ttt{source} property key to specify a different signal as input for the
statistic. The following example assumes that the C++ code emits a \ttt{qlen}
signal and declares a \ttt{queueLength} statistic based on that:
...
Note that the \ttt{source=qlen} property key has been added to specify
the \ttt{qlen} signal as the input for the statistic. Additionally, a
signal declaration (\fprop{@signal} property) has been added for the
...
C++ code emits a \ttt{drop} signal every time a packet is dropped.
Here, \ttt{count()} is a \textit{result filter}.

\begin{note}
Starting from {\opp} 4.4, items containing parentheses (e.g.,
\ttt{count(drop)}) no longer need to be enclosed in quotation marks.
\end{note}
...
This assumes that the C++ code emits a \ttt{pkdrop} signal with a
\cclass{cPacket} pointer as the value. Based on that signal, it records
the total number of bytes dropped as a scalar and optionally as a vector.
The \ttt{packetBytes()} filter extracts the number of bytes from each packet
using the \ffunc{getByteLength()} method in \cclass{cPacket}, and the
\ttt{sum()} filter sums up the values.
...
\cclass{cPacket*}-valued signal as input. However, some of the computations
...
\begin{description}
  \item[source]: Defines the input for the recorders (see the \ttt{record=} key).
  ...
  \item[record]: Contains a list of recording modes, separated by commas.
     Recording modes define how to record the source (see the \ttt{source=} key).
...
%XXX clarify whether needed/used:  \item[type]: XXX int, double, enum
  \item[enum]: Defines symbolic names for various integer signal values.
      ...
      separated by commas. For example: \ttt{"IDLE=1,BUSY=2,DOWN=3"}.
%XXX TODO implement:  \item[enumName]: xxx
  \item[checkSignals]: Controls whether signal declarations (\fprop{@signal})
      are checked for the signals used by this statistic. Accepted values are
      \ttt{true} and \ttt{false}. When not specified, the global setting
      (the \fconfig{check-signals} configuration option) is used.
  \item[autoWarmupFilter]: Controls whether a warmup period filter is
      automatically inserted at the front of the result filter chain.
      Accepted values are \ttt{true} and \ttt{false}. When not specified
      (or set to \ttt{true}), the warmup filter is added if the configured
      warmup period is nonzero. Setting it to \ttt{false} disables the
      automatic warmup filter for this statistic.
\end{description}
...
by executing the \ttt{opp\_run -h resultfilters} and \ttt{opp\_run
...
would have exactly the same name. Examples: \ttt{"Dropped Bytes, sum"},
\ttt{"Dropped Bytes, vector(sum)"}.
...
To fully understand \ttt{source} and \ttt{record}, it is useful
to see how result recording is set up.

When a module or channel is created in the simulation, the {\opp} runtime
checks the \ttt{@statistic} properties on its NED declaration and adds
...
\ttt{sum} filter passes on the sum of the values received on its input to
its output, while the \ttt{sum} recorder computes the sum of the received values
...
configuration option. For example, specify \ttt{--debug-statistics-recording=true}
on the command line.
\end{hint}
...
The \ttt{demux} result filter in {\opp} provides a mechanism for
...
The \ttt{demux} filter works by demultiplexing its input into several
outputs, dynamically creating new outputs as required. The filter uses the name
string of the \ttt{details} object associated with the emitted signal as the
...
a statistic that uses the \ttt{demux} filter:
...
With the \ttt{demux} filter, the generated statistic names will include
the demux label (i.e., the sender's name), resulting in statistic names such as:

\begin{itemize}
    \item \ttt{bytesReceivedPerSender:Sender1:sum}
    \item \ttt{bytesReceivedPerSender:Sender2:sum}
    \item ...
\end{itemize}

Each statistic records the sum of the bytes received from its corresponding sender,
...
and setting up \fprop{@statistic}-style result recording for each.
...
\ffunc{registerSignal()} and, in addition, inform {\opp} to set up statistics
recording for it as described by the \fprop{@statisticTemplate} property.
This can be done by calling \ffunc{getEnvir()->addResultRecorders()}.
...
name and index of the property will also be ignored. (In the case of \fprop{@statistic},
the index holds the result name, but here the name is explicitly specified
in the \ttt{statisticName} parameter.)

When recording multiple signals using a common \fprop{@statisticTemplate},
you may want the titles of the recorded statistics to differ for each signal.
This can be achieved by using dollar variables in the \ttt{title} key of the
\fprop{@statisticTemplate}. The following variables are available:

\begin{itemize}
  \item \ttt{\$name}: name of the statistic
  \item \ttt{\$component}: component's full path
  \item \ttt{\$mode}: recording mode
  \item \ttt{\$namePart[0-9]+}: the given part of the statistic name when split
...
As an alternative to using \fprop{@statisticTemplate} and \ffunc{addResultRecorders()},
...
\begin{note}
It is important to know that \ttt{@statistic} implements warm-up period
...
that take \ttt{long}, \ttt{double}, and \ttt{simtime\_t} are typically used.
...
to associate values with past timestamps, as is done with the \ffunc{recordWithTimestamp()}
method of \cclass{cOutVector} (see \ref{sec:sim-lib:coutvector}). This situation
...
that contains a $(timestamp, value)$ pair, and use the \ffunc{emit(simsignal\_t, cObject*)}
method to emit it. The \cclass{cTimestampedValue} class provides this functionality,
with two public data members: \ttt{time} (of type \ttt{simtime\_t}) and
\ttt{value} (of type \ttt{double}). It also has a convenience constructor that
takes these two values.

\begin{note}
\cclass{cTimestampedValue} is not part of the signal mechanism per se. However,
the result recording listeners provided by {\opp} are designed to understand
\cclass{cTimestampedValue} and know how to handle it.
\end{note}
...
If performance is critical, you can make the \cclass{cTimestampedValue} object a
...
(i.e., \ttt{const char *} and \ttt{cObject *}) may also be recorded as results.
...
\begin{itemize}
  ...
  \item Objects that can be cast to \cclass{cITimestampedValue} are recorded
     using the \ffunc{getSignalTime()} and \ffunc{getSignalValue()} methods of the class.
  \item Other objects are recorded as 1.0, except for \ttt{nullptr}, which is recorded as 0.0.
\end{itemize}

\cclass{cITimestampedValue} is a C++ interface that can be used as an additional
...
The \ffunc{getSignalValue()} function is pure virtual (i.e., it must return some value),
but the \ffunc{getSignalTime()} function has a default implementation that
returns the current simulation time. Note that the \ttt{signalID} argument
...
or recorder can then be used in the \ttt{source=} and \ttt{record=} attributes
of \fprop{@statistic} properties, just like the built-in ones.

Result filters must be subclassed from \cclass{cResultFilter} or one
...
Similarly, a result recorder must be subclassed from \cclass{cResultRecorder} or
the more specific \cclass{cNumericResultRecorder} class, and be registered
using the \fmac{Register\_ResultRecorder(NAME, CLASSNAME)} macro.
