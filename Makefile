...
create-shortcuts:
ifeq ($(PLATFORM),linux)
	@printf "[Desktop Entry]\nType=Application\nExec=$(OMNETPP_ROOT)/setenv opp_ide\nPath=$(OMNETPP_ROOT)\nIcon=$(OMNETPP_ROOT)/images/logo/logo128.png\nName=$(OMNETPP_PRODUCT) $(OMNETPP_VERSION) IDE\nCategories=Development\n" >$(OMNETPP_RELEASE)-ide.desktop && chmod +x $(OMNETPP_RELEASE)-ide.desktop
	@printf "[Desktop Entry]\nType=Application\nExec=$(OMNETPP_ROOT)/setenv bash\nPath=$(OMNETPP_ROOT)\nTerminal=true\nIcon=$(OMNETPP_ROOT)/images/logo/logo128s.png\nName=$(OMNETPP_PRODUCT) $(OMNETPP_VERSION) Shell\nCategories=Development\n" >$(OMNETPP_RELEASE)-shell.desktop && chmod +x $(OMNETPP_RELEASE)-shell.desktop
else ifeq ($(PLATFORM),win32)
	@-src/utils/shortcut.cmd -linkfile "$(OMNETPP_ROOT)/$(OMNETPP_PRODUCT) $(OMNETPP_VERSION) IDE.lnk" -target "$(OMNETPP_ROOT)/opp_shell.cmd" -linkarguments opp_ide -iconlocation "$(OMNETPP_ROOT)/images/logo/logo128.ico" -workingdirectory "$(OMNETPP_ROOT)" -windowstyle 7
	@-src/utils/shortcut.cmd -linkfile "$(OMNETPP_ROOT)/$(OMNETPP_PRODUCT) $(OMNETPP_VERSION) Shell.lnk" -target "$(OMNETPP_ROOT)/opp_shell.cmd" -iconlocation "$(OMNETPP_ROOT)/images/logo/logo128s.ico" -workingdirectory "$(OMNETPP_ROOT)" -windowstyle 7
endif
