...
{
    this->cfg = dynamic_cast<cConfigurationEx *>(cfg);
    if (!this->cfg)
        throw cRuntimeError("cRngManager: Configuration object is not a cConfigurationEx");
    ...
    for (int i = 0; i < numRNGs; i++) {
        ...
        rngs[i]->initialize(seedset, i, numRNGs, envir->getParsimProcId(), envir->getParsimNumPartitions(), cfg);
    }
}
...
void cRngManager::configureRngs(cComponent *component)
{
    ...
    for (auto suffix : suffixes) {
        ...
        try {
            ...
            if (physRng >= getTotalNumRngs())
                throw cRuntimeError("RNG index %d out of range (num-rngs=%d)", physRng, getTotalNumRngs());
            if (modRng >= mapsize) {
                if (modRng >= 100)
...
cRNG *cRngManager::getRng(const cComponent *component, int k)
{
    ...
    return getGlobalRng(rngId);
}

int cRngManager::getTotalNumRngs() const
{
    return numRNGs;
}

cRNG *cRngManager::getGlobalRng(int rngId)
{
