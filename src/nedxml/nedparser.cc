...
const char *NedParser::getBuiltInDeclarations()
{
    return
        ...
        "    @statistic[averageUtilization](source=timeavg(channelBusy);record=last?);\n"
        ...
        "    @statistic[averageThroughput](source=sumPerDuration(packetBits(messageSent));record=last?;unit=bps);\n"
        "    bool disabled @mutable = default(false);\n"
        "    double delay @mutable = default(0s) @unit(s); // propagation delay\n"
