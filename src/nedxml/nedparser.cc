...
const char *NedParser::getBuiltInDeclarations()
{
    return
        ...
        "    @statistic[messages](title=\"messages sent\";source=constant1(messageSent);record=count?;interpolationmode=none);\n"
        "    @statistic[messagesDiscarded](title=\"messages discarded\";source=constant1(messageDiscarded);record=count?;interpolationmode=none);\n"
        ...
        "    @signal[messageSent](type=omnetpp::cMessage);\n"
        "    @signal[messageDiscarded](type=omnetpp::cMessage);\n"
        "    @statistic[busy](title=\"channel busy state\";source=channelBusy;record=vector?;interpolationmode=sample-hold);\n"
        "    @statistic[averageUtilization](title=\"average channel utilization\";source=timeavg(channelBusy);record=last?);\n"
        "    @statistic[packets](title=\"packets sent\";source=constant1(messageSent);record=count?;interpolationmode=none);\n"
        "    @statistic[packetBytes](title=\"packet bytes sent\";source=packetBytes(messageSent);record=sum?;unit=B;interpolationmode=none);\n"
        "    @statistic[packetsDiscarded](title=\"packets discarded\";source=constant1(messageDiscarded);record=count?;interpolationmode=none);\n"
        "    @statistic[averageThroughput](title=\"average throughput\";source=sumPerDuration(packetBits(messageSent));record=last?;unit=bps);\n"
        "    bool disabled @mutable = default(false);\n"
        "    double delay @mutable = default(0s) @unit(s); // propagation delay\n"
