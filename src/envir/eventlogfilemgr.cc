...
void EventlogFileManager::componentMethodBegin(cComponent *from, cComponent *to, const char *methodText)
{
    if (isEventRecordingEnabled && isMethodCallRecordingEnabled) {
        FileLockAcquirer fileLockAcquirer(fileLock, FILE_LOCK_EXCLUSIVE);
        EventLogWriter::recordComponentMethodBeginEntry_sm_tm_m(feventlog, from ? from->getId() : -1, to->getId(), opp_nulltoempty(methodText));
        entryIndex++;
    }
}
