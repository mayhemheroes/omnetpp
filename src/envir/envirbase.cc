...
#define DEFAULT_OUTPUT_SCALAR_PRECISION    "17"
#define DEFAULT_OUTPUT_VECTOR_PRECISION    "17"
...
extern cConfigOption *CFGID_RNGMANAGER_CLASS; // registered in csimulation.cc
...
Register_PerObjectConfigOption(CFGID_VECTOR_RECORD_EMPTY, "vector-record-empty", KIND_VECTOR, CFG_BOOL, "true", "Whether output vectors that have no data recorded into them should appear in the result file. Turning off this option will delay writing the vector declaration into the result file until there is data recorded into the vector. Usage: `<module-full-path>.<vector-name>.vector-record-empty=true/false`.\nExample: `**.ping.roundTripTime:vector.vector-record-empty=false`");
...
void EnvirBase::readPerRunOptions()
{
    ...
    getSimulation()->setRngManager(rngManager);
...
{
    return getSimulation()->getRngManager()->getTotalNumRngs();
}

cRNG *EnvirBase::getRNG(int k)
{
    return getSimulation()->getRngManager()->getGlobalRng(k);
}
